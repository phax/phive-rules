...
public final class MainExtractTestFiles24
{
  private static final Logger LOGGER = LoggerFactory.getLogger (MainExtractTestFiles24.class);

  public static void main (final String [] args)
  {
    // TODO edit the following 2 lines for a new version
    final File fBasePath = new File ("docs/2.4/EN/Examples").getAbsoluteFile ();
    final File fTargetPath = new File ("src/test/resources/external/test-files/2.4").getAbsoluteFile ();
    ...
    for (final EZugferdProfile eProfile : EZugferdProfile.values ())
    {
      ...
      for (final File fExampleFolder : aExampleFolders)
      {
        ...
        for (final File fExample : aXMLFiles)
        {
          ...
          if (false)
            LOGGER.info ("  " + fExample.getName () + " - " + nExampleIndex);

          if (eProfile == EZugferdProfile.EXTENDED &&
              fExample.getName ().equals ("Extended___SubInvoiceLines_Kaffee_Bundle_Set_Bsp4__.xml"))
          {
