...
<!--KEYS AND FUNCTIONS-->
<xsl:function as="xs:decimal" name="u:vatBase">
    <xsl:param as="xs:string" name="cat" />
    <xsl:param as="element()*" name="lines" />
    <xsl:param as="element()*" name="allow" />
    <xsl:param as="element()*" name="charge" />
  ...
  <xsl:function as="xs:decimal" name="u:to-decimal">
        <xsl:param name="val" />
  ...
  <xsl:function as="xs:boolean" name="u:isZeroOrEmpty">
        <xsl:param as="node()*" name="n" />
...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="PINT Oman E-Invoice Validation Rules (IBR-OM)">
      <xsl:comment>
      ...
      <svrl:active-pattern>
        ...
        <xsl:attribute name="name">PINT Oman - business rules and calculation consistency</xsl:attribute>
      </svrl:active-pattern>
      ...
      <svrl:active-pattern>
        ...
        <xsl:attribute name="name">PINT Oman - codelist conformance</xsl:attribute>
      </svrl:active-pattern>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>PINT Oman E-Invoice Validation Rules (IBR-OM)</svrl:text>
  <xsl:param name="doc" select="/*[local-name()='Invoice' or local-name()='CreditNote']" />
  <xsl:param name="lines" select="$doc/cac:InvoiceLine | $doc/cac:CreditNoteLine" />
  <xsl:param name="allowances" select="$doc/cac:AllowanceCharge[cbc:ChargeIndicator='false']" />
  <xsl:param name="charges" select="$doc/cac:AllowanceCharge[cbc:ChargeIndicator='true']" />
  <xsl:param name="invoiceType" select="normalize-space(($doc/cbc:InvoiceTypeCode | $doc/cbc:CreditNoteTypeCode)[1])" />
  <xsl:param name="txnType" select="normalize-space(($doc/cbc:InvoiceTypeCode/@name | $doc/cbc:CreditNoteTypeCode/@name)[1])" />
  <xsl:param name="isValidBitString" select="matches($txnType, '^[01]{20}$')" />
  <xsl:param name="txnSafe" select="if ($isValidBitString) then $txnType else 'XXXXXXXXXXXXXXXXXXXX'" />
  <xsl:param name="invoiceCurrency" select="normalize-space($doc/cbc:DocumentCurrencyCode)" />
  <xsl:param name="taxCurrency" select="normalize-space($doc/cbc:TaxCurrencyCode)" />
  <xsl:param name="exchangeRate" select="             if ($doc/cac:TaxExchangeRate/cbc:CalculationRate castable as xs:decimal)             then xs:decimal($doc/cac:TaxExchangeRate/cbc:CalculationRate)             else ()" />
  ...
  <xsl:param name="baseType" select="if ($isFullTax) then 'Full Invoice' else if ($isSimplified) then 'Simplified Invoice' else 'Unknown Type'" />
  ...
  <xsl:param name="txnTypeLabel" select="concat($baseType, if (normalize-space($subTypes) != '') then concat(' (', $subTypes, ')') else '')" />
  <xsl:param name="standardVatRate" select="xs:decimal('5')" />
  <xsl:param name="amountTolerance" select="xs:decimal('0.01')" />
  <xsl:param name="sevenDecimalScale" select="xs:decimal('10000000')" />
  <xsl:param name="baisaTolerance" select="xs:decimal('0.001')" />
  <xsl:param name="lineCount" select="count($lines)" />
  <xsl:param name="baisaCap" select="$baisaTolerance * $lineCount" />

<!--PATTERN Aligned-om-rulesPINT Oman - business rules and calculation consistency-->
<svrl:text>PINT Oman - business rules and calculation consistency</svrl:text>

	<!--RULE -->
<xsl:template match="/" mode="M50" priority="1012">
    <svrl:fired-rule context="/" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="*[local-name() = ('Invoice','CreditNote')                        and namespace-uri() = (                             'urn:oasis:names:specification:ubl:schema:xsd:Invoice-2',                             'urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2')]" />
      <xsl:otherwise>
        <svrl:failed-assert test="*[local-name() = ('Invoice','CreditNote') and namespace-uri() = ( 'urn:oasis:names:specification:ubl:schema:xsd:Invoice-2', 'urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2')]">
          <xsl:attribute name="id">BTOM-PRE-001</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-BTOM-PRE-001">
<svrl:text>
          ...
          <svrl:text>
                [BTOM-PRE-001] - Document root MUST be a UBL 2.1 Invoice or CreditNote in the correct namespace. Found '<xsl:text />
            <xsl:value-of select="local-name(*[1])" />
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice | cn:CreditNote" mode="M50" priority="1011">
    <svrl:fired-rule context="ubl:Invoice | cn:CreditNote" />

		<!--REPORT information-->
<xsl:if test="true()">
      <svrl:successful-report test="true()">
        <xsl:attribute name="id">BTOM-META-001</xsl:attribute>
        <xsl:attribute name="role">information</xsl:attribute>
        <xsl:attribute name="location">
          <xsl:apply-templates mode="schematron-select-full-path" select="." />
        </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-BTOM-META-001">
<svrl:text>
        ...
        <svrl:text>
                [PINT-OM-META] Ruleset version 1.0.1 - Authority: Tax Authority Oman.
            </svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$txnType != '' and $isValidBitString and contains($txnType, '1')">
          <xsl:attribute name="id">ALIGNED-IBRP-000-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-000">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="starts-with($customizationID, 'urn:peppol:pint:selfbilling-1@om-1')" />
      <xsl:otherwise>
        <svrl:failed-assert test="starts-with($customizationID, 'urn:peppol:pint:selfbilling-1@om-1')">
          <xsl:attribute name="id">ALIGNED-IBRP-001-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-001">
<svrl:text>
          ...
          <svrl:text>[ALIGNED-IBRP-001-OM] - Specification identifier (IBT-024) MUST start with the value 'urn:peppol:pint:selfbilling-1@om-1'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="starts-with($profileID, 'urn:peppol:bis:selfbilling')">
          <xsl:attribute name="id">ALIGNED-IBRP-002-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-002">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($taxCurrency != '') or $taxCurrency = 'OMR'">
          <xsl:attribute name="id">ALIGNED-IBRP-003-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-003">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="exists(cbc:IssueTime)">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-016">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not( $lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID='E'] or $allowances[cac:TaxCategory/cbc:ID='E'] or $charges[cac:TaxCategory/cbc:ID='E'] ) or ( (not($isSimplified) and count($doc/cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='E']) = 1) or ($isSimplified and count($doc/cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='E']) &lt;= 1) )">
          <xsl:attribute name="id">ALIGNED-IBRP-E-01-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-E-01">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not( $lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID='O'] or $allowances[cac:TaxCategory/cbc:ID='O'] or $charges[cac:TaxCategory/cbc:ID='O'] ) or ( (not($isSimplified) and count($doc/cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='O']) = 1) or ($isSimplified and count($doc/cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='O']) &lt;= 1) )">
          <xsl:attribute name="id">ALIGNED-IBRP-O-01-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-O-01">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(                         $lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID='Z'] or                         $allowances[cac:TaxCategory/cbc:ID='Z'] or                         $charges[cac:TaxCategory/cbc:ID='Z']                     ) or (                         (not($isSimplified) and count(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='Z']) = 1) or                         ($isSimplified and count(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='Z']) &lt;= 1)                     )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not( $lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID='Z'] or $allowances[cac:TaxCategory/cbc:ID='Z'] or $charges[cac:TaxCategory/cbc:ID='Z'] ) or ( (not($isSimplified) and count(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='Z']) = 1) or ($isSimplified and count(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='Z']) &lt;= 1) )">
          <xsl:attribute name="id">ALIGNED-IBRP-Z-01-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-Z-01">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="count( cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme [cac:TaxScheme/cbc:ID = 'VAT'] /cbc:CompanyID ) &lt;= 1">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-SR-12">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="every $rate in distinct-values(($lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID='S']/cac:Item/cac:ClassifiedTaxCategory/cbc:Percent, $allowances[cac:TaxCategory/cbc:ID='S']/cac:TaxCategory/cbc:Percent, $charges[cac:TaxCategory/cbc:ID='S']/cac:TaxCategory/cbc:Percent)) satisfies count(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID='S' and xs:decimal(cac:TaxCategory/cbc:Percent) = xs:decimal($rate)]) = 1">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-S-01">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID = 'S']) or                         (every $line in $lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID = 'S'] satisfies                         ($line/cac:Item/cac:ClassifiedTaxCategory/cbc:Percent/normalize-space(.) castable as xs:decimal and                         xs:decimal($line/cac:Item/cac:ClassifiedTaxCategory/cbc:Percent) = $standardVatRate))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID = 'S']) or (every $line in $lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID = 'S'] satisfies ($line/cac:Item/cac:ClassifiedTaxCategory/cbc:Percent/normalize-space(.) castable as xs:decimal and xs:decimal($line/cac:Item/cac:ClassifiedTaxCategory/cbc:Percent) = $standardVatRate))">
          <xsl:attribute name="id">ALIGNED-IBRP-S-05-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-S-05">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="string-length($txnType) = 20 and matches($txnType, '^[01]{20}$')">
          <xsl:attribute name="id">IBR-001-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-001">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="matches(normalize-space(cbc:UUID), '^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-5[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$')">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-002">
<svrl:text>
          ...
          <svrl:text>[IBR-002-OM] - The unique identifier (“UUID”) (BTOM-002) must be provided on the invoice and MUST be a valid UUID version 5.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$invoiceCurrency = 'OMR' or exists($exchangeRate)">
          <xsl:attribute name="id">IBR-004-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-004">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($taxCurrency='OMR' and $invoiceCurrency!='OMR')                         or (exists($exchangeRate)                             and $exchangeRate = round($exchangeRate * $sevenDecimalScale) div $sevenDecimalScale)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($taxCurrency='OMR' and $invoiceCurrency!='OMR') or (exists($exchangeRate) and $exchangeRate = round($exchangeRate * $sevenDecimalScale) div $sevenDecimalScale)">
          <xsl:attribute name="id">IBR-005-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-005">
<svrl:text>
            Currency exchange rate (BTOM-003)
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$isImportGoods or $isImportRCM or $isProfitMarginSelf or cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme[cac:TaxScheme/cbc:ID='VAT']/cbc:CompanyID">
          <xsl:attribute name="id">IBR-006-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-006">
<svrl:text>
            Seller VAT identifier (IBT-031)
          ...
          <svrl:text>[IBR-006-OM] - Seller tax identifier (IBT-031) MUST be mandatory in all cases except when Invoice transaction type (BTOM-001) is an invoice for import of goods (00000000000010000000), import of service RCM (00000000100000000000) or profit margin self invoice (00000000001000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isImportGoods or $isImportRCM or $isProfitMarginSelf or $isSpecialZone) or cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID">
          <xsl:attribute name="id">IBR-007-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-007">
<svrl:text>
            TxnType: '<xsl:text />
              <xsl:value-of select="$txnType" />
              <xsl:text />'
          ...
          <svrl:text>[IBR-007-OM] - Seller identifier (IBT-029) Scheme identifier (IBT-029-1) must be provided when Invoice transaction type (BTOM-001) is an invoice for import of goods (00000000000010000000) or import of service RCM (00000000100000000000) or profit margin self invoice (00000000001000000000) or Special zone supplies (00000000000001000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:StreetName and cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:AdditionalStreetName and cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:CityName and cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:PostalZone and cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cac:AddressLine/cbc:Line">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-010">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="cac:AccountingSupplierParty/cac:Party/cac:Contact/cbc:Telephone">
          <xsl:attribute name="id">IBR-011-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-011">
<svrl:text>
            Expected:
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isExport and $lines//cbc:TaxExemptionReasonCode = 'VATZR-OM-09') or cac:Delivery/cac:DeliveryLocation/cac:Address/cac:Country/cbc:IdentificationCode != 'OM'">
          <xsl:attribute name="id">IBR-012-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-012">
<svrl:text>
          ...
          <svrl:text>[IBR-012-OM] - Deliver to country code (IBT-080) must not be 'OM' if invoice transaction type (BTOM-001) is export invoice (00000010000000000000) and at least one VAT exemption reason code (IBT-121) is 'Export of service (VATZR-OM-09)'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isExport and $lines//cbc:TaxExemptionReasonCode ='VATZR-OM-12')                          or (exists($doc/cac:AdditionalDocumentReference                             [not(cbc:DocumentTypeCode='130')]                             [cbc:ID and cbc:UUID]))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isExport and $lines//cbc:TaxExemptionReasonCode ='VATZR-OM-12') or (exists($doc/cac:AdditionalDocumentReference [not(cbc:DocumentTypeCode='130')] [cbc:ID and cbc:UUID]))">
          <xsl:attribute name="id">IBR-013-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-013">
<svrl:text>
          ...
          <svrl:text>[IBR-013-OM] - Supporting document reference (IBT-122) and Supporting document UUID (BTOM-023) must be provided if invoice transaction type (BTOM-001) is export invoice (00000010000000000000) and at least one VAT exemption reason code (IBT-121) is 'Re-export of goods (VATZR-OM-12)'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isExport) or cac:Delivery/cac:DeliveryLocation/cac:Address/cac:Country/cbc:IdentificationCode">
          <xsl:attribute name="id">IBR-014-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-014">
<svrl:text>
          ...
          <svrl:text>[IBR-014-OM] - Deliver to country code (IBT-080) must be provided if invoice transaction type (BTOM-001) is export invoice (00000010000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isThirdParty) or ( count(cac:AccountingSupplierParty/cac:Party/cac:AgentParty) = 1 and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PartyName/cbc:Name and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PartyTaxScheme[cac:TaxScheme/cbc:ID='VAT']/cbc:CompanyID and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PartyTaxScheme/cac:TaxScheme/cbc:ID and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PostalAddress/cbc:StreetName and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PostalAddress/cbc:AdditionalStreetName and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PostalAddress/cac:AddressLine/cbc:Line and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PostalAddress/cbc:CityName and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PostalAddress/cbc:PostalZone and cac:AccountingSupplierParty/cac:Party/cac:AgentParty/cac:PostalAddress/cac:Country/cbc:IdentificationCode)">
          <xsl:attribute name="id">IBR-015-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-015">
<svrl:text>
          ...
          <svrl:text>[IBR-015-OM] - Third Party Name (BTOM-005), Third Party VATIN (BTOM-006), VAT Scheme Code (BTOM-06-1), Third Party Address Line 1 (BTOM-007), Third Party Address Line 2 (BTOM-008), Third Party Address Line 3 (BTOM-009), Third party city (BTOM-010), Third party postal code (BTOM-011) and Third Party Country Code (BTOM-13) must be provided when Invoice transaction type (BTOM-001) is Third-party Invoice (00010000000000000000) and MUST occur only once.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isFullTax or $isThirdParty or $isSummary or $isContinuous or $isExport or $isProfitMargin or $isEcommerce) or (cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID or cac:AccountingCustomerParty/cac:Party/cac:PartyTaxScheme[cac:TaxScheme/cbc:ID='VAT']/cbc:CompanyID)">
          <xsl:attribute name="id">IBR-016-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-016">
<svrl:text>
          ...
          <svrl:text>[IBR-016-OM] - Either Buyer identifier (IBT-046) or Buyer VATIN (IBT-048) MUST be present when the Invoice transaction type (BTOM-001) is a Full tax invoice (10000000000000000000) OR Third-party Invoice (00010000000000000000) OR Summary invoice (00001000000000000000) OR Continuous supply (00000100000000000000) OR Export Invoice (00000010000000000000) OR Profit margin invoice (00000000010000000000) OR E-commerce supplies (00000000000100000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isSelfBilled or $isImportRCM or $isProfitMarginSelf or $isImportGoods)                   or cac:AccountingCustomerParty/cac:Party/cac:PartyTaxScheme[cac:TaxScheme/cbc:ID='VAT']/cbc:CompanyID" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSelfBilled or $isImportRCM or $isProfitMarginSelf or $isImportGoods) or cac:AccountingCustomerParty/cac:Party/cac:PartyTaxScheme[cac:TaxScheme/cbc:ID='VAT']/cbc:CompanyID">
          <xsl:attribute name="id">IBR-017-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-017">
<svrl:text>
            Buyer VATIN (IBT-048)
          ...
          <svrl:text>[IBR-017-OM] - Buyer VATIN (IBT-048) MUST be present when the Invoice transaction type (BTOM-001) is a Self-billed Invoice/credit note (00100000000000000000) or Invoice for import of services for RCM (00000000100000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isFullTax or $isThirdParty or $isSelfBilled or $isExport or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isImportGoods or $isSpecialZone or $isSummary)                   or (cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:StreetName                       and cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:AdditionalStreetName                       and cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CityName                       and cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:PostalZone                       and cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cac:AddressLine/cbc:Line)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isFullTax or $isThirdParty or $isSelfBilled or $isExport or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isImportGoods or $isSpecialZone or $isSummary) or (cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:StreetName and cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:AdditionalStreetName and cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CityName and cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:PostalZone and cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cac:AddressLine/cbc:Line)">
          <xsl:attribute name="id">IBR-019-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-019">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isSelfBilled or $isImportRCM or $isProfitMarginSelf or $isImportGoods)                   or cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode='OM'" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSelfBilled or $isImportRCM or $isProfitMarginSelf or $isImportGoods) or cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode='OM'">
          <xsl:attribute name="id">IBR-020-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-020">
<svrl:text>
          ...
          <svrl:text>[IBR-020-OM] - Buyer country code (IBT-055) MUST be 'OM' when the Invoice transaction type (BTOM-001) is a Self-billed Invoice/credit note (00100000000000000000) or Invoice for import of services for RCM (00000000100000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($invoiceType='381' or $invoiceType='383' or $invoiceType='261') or cac:BillingReference/cac:InvoiceDocumentReference/cbc:DocumentStatusCode">
          <xsl:attribute name="id">IBR-023-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-023">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($invoiceType='381' or $invoiceType='383' or $invoiceType='261') or (cac:BillingReference/cac:InvoiceDocumentReference/cbc:ID and cac:BillingReference/cac:InvoiceDocumentReference/cbc:IssueDate and cac:BillingReference/cac:InvoiceDocumentReference/cbc:UUID)">
          <xsl:attribute name="id">IBR-032-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-032">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$invoiceCurrency = 'OMR' or ($taxCurrency != '')">
          <xsl:attribute name="id">IBR-034-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-034">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSummary) or (substring(cac:InvoicePeriod/cbc:StartDate,1,7) = substring(cac:InvoicePeriod/cbc:EndDate,1,7))">
          <xsl:attribute name="id">IBR-036-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-036">
<svrl:text>
          ...
          <svrl:text>[IBR-036-OM] - Invoicing period Start date (IBT-073) and Invoicing period end date (IBT-074) provided must belong to the same calendar month where Invoice transaction type code (BTOM-001) is a summary invoice (00001000000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSummary or $isContinuous) or (cac:InvoicePeriod/cbc:StartDate and cac:InvoicePeriod/cbc:EndDate)">
          <xsl:attribute name="id">IBR-037-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-037">
<svrl:text>
          ...
          <svrl:text>[IBR-037-OM] - Invoicing period start date (IBT-073) and the Invoicing period end date (IBT-074) must be provided where Invoice transaction type code (BTOM-001) is a summary invoice (00001000000000000000) or Continuous supply (00000100000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isEcommerce) or (cac:Delivery/cac:DeliveryLocation/cac:Address/cbc:StreetName and cac:Delivery/cac:DeliveryLocation/cac:Address/cbc:AdditionalStreetName and cac:Delivery/cac:DeliveryLocation/cac:Address/cbc:CityName and cac:Delivery/cac:DeliveryLocation/cac:Address/cbc:PostalZone and cac:Delivery/cac:DeliveryLocation/cac:Address/cac:Country/cbc:IdentificationCode and cac:Delivery/cac:DeliveryLocation/cac:Address/cac:AddressLine/cbc:Line)">
          <xsl:attribute name="id">IBR-040-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-040">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="substring($txnType,1,1)='1' or substring($txnType,2,1)='1'">
          <xsl:attribute name="id">IBR-043-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-043">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(exists(cac:LegalMonetaryTotal/cbc:PrepaidAmount))                         or (exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:ID)                             and exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:UUID))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(exists(cac:LegalMonetaryTotal/cbc:PrepaidAmount)) or (exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:ID) and exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:UUID))">
          <xsl:attribute name="id">IBR-058-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-058">
<svrl:text>
            ...
            Originator ID present: '<xsl:text />
              <xsl:value-of select="exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:ID)" />
              <xsl:text />'
            Value: '<xsl:text />
              <xsl:value-of select="cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:ID" />
              <xsl:text />'

            Originator UUID present: '<xsl:text />
              <xsl:value-of select="exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:UUID)" />
              <xsl:text />'
            Value: '<xsl:text />
              <xsl:value-of select="cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:UUID" />
              <xsl:text />'
        </svrl:text>
</svrl:diagnostic-reference>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($exchangeRate) or (cac:TaxExchangeRate/cbc:SourceCurrencyCode=$invoiceCurrency and cac:TaxExchangeRate/cbc:TargetCurrencyCode=$taxCurrency)">
          <xsl:attribute name="id">IBR-059-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-059">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($invoiceCurrency != 'OMR' and $taxCurrency = 'OMR')                         or (                             $exchangeRate castable as xs:decimal and                             u:slack(                                 xs:decimal((cac:TaxTotal[cbc:TaxAmount/@currencyID = $taxCurrency]/cbc:TaxAmount)[1]),                                 xs:decimal($exchangeRate * (cac:TaxTotal[cbc:TaxAmount/@currencyID = $invoiceCurrency]/cbc:TaxAmount)[1]),                                 0.01                             )                         )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($invoiceCurrency != 'OMR' and $taxCurrency = 'OMR') or ( $exchangeRate castable as xs:decimal and u:slack( xs:decimal((cac:TaxTotal[cbc:TaxAmount/@currencyID = $taxCurrency]/cbc:TaxAmount)[1]), xs:decimal($exchangeRate * (cac:TaxTotal[cbc:TaxAmount/@currencyID = $invoiceCurrency]/cbc:TaxAmount)[1]), 0.01 ) )">
          <xsl:attribute name="id">IBR-065-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-065">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($invoiceCurrency != 'OMR' and $doc//cac:TaxCategory/cbc:ID = 'S') or exists( $doc/cac:TaxTotal[cbc:TaxAmount/@currencyID = $taxCurrency] /cac:TaxSubtotal[ cbc:TaxAmount/@currencyID = $taxCurrency and cac:TaxCategory/cbc:ID = 'S' and cbc:TaxAmount and cac:TaxCategory/cbc:Percent ] )">
          <xsl:attribute name="id">IBR-066-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-066">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isProfitMargin or $isProfitMarginSelf)                         or (                             cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:DocumentDescription castable as xs:decimal                             and u:slack(                                 xs:decimal(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:DocumentDescription),                                 xs:decimal(sum($lines/cac:ItemPriceExtension/cbc:Amount/xs:decimal(.))),                                 0.01                             )                         )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isProfitMargin or $isProfitMarginSelf) or ( cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:DocumentDescription castable as xs:decimal and u:slack( xs:decimal(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:DocumentDescription), xs:decimal(sum($lines/cac:ItemPriceExtension/cbc:Amount/xs:decimal(.))), 0.01 ) )">
          <xsl:attribute name="id">IBR-082-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-082">
<svrl:text>
          ...
          <svrl:text>[IBR-082-OM] - When Invoice transaction type (BTOM-001) is Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000), then Total Amount Due (BTOM-020), should be provided and is mandatory and must be the sum of Total amount including VAT (BTOM-017).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isImportGoods) or ( cac:Delivery/cac:Shipment/cac:Delivery/cbc:ActualDeliveryDate and cac:Delivery/cac:Shipment/cbc:ID and cac:Delivery/cac:DeliveryTerms/cbc:ID )">
          <xsl:attribute name="id">IBR-085-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-085">
<svrl:text>
          ...
          <svrl:text>[IBR-085-OM] - If invoice transaction type (BTOM-001) is 'Import of Goods' (00000000000010000000), then Import date (BTOM-030), Custom Declaration number (BTOM-021) and Incoterms (BTOM-022).MUST be present.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isProfitMarginSelf)                 or cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode='OM'" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isProfitMarginSelf) or cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode='OM'">
          <xsl:attribute name="id">IBR-087-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-087">
<svrl:text>
          ...
          <svrl:text>[IBR-087-OM] - In case Invoice transaction type (BTOM-001) is 'Profit Margin Self-Invoice' (00000000001000000000), Seller Country Code (IBT-040) MUST be 'OM'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($taxCurrency != '') or (some $total in cac:TaxTotal satisfies ($total/cbc:TaxAmount/@currencyID = $taxCurrency and (every $sub in $total/cac:TaxSubtotal/cbc:TaxAmount satisfies $sub/@currencyID = $taxCurrency)) )">
          <xsl:attribute name="id">IBR-136-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-136">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSelfBilled and ($isThirdParty or $isExport or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))">
          <xsl:attribute name="id">IBR-138-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-138">
<svrl:text>
          ...
          <svrl:text>[IBR-138-OM] - Invoice transaction type (BTOM-001) cannot be Self-billed Invoice/credit note (00100000000000000000) if Invoice transaction type (BTOM-001) is Third-party Invoice (00010000000000000000) or Export Invoice (00000010000000000000) or Import of services for RCM (00000000100000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isThirdParty and $isSelfBilled)">
          <xsl:attribute name="id">IBR-139-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-139">
<svrl:text>
          ...
          <svrl:text>[IBR-139-OM] - Invoice transaction type (BTOM-001) cannot be Third-party Invoice (00010000000000000000) if Invoice transaction type (BTOM-001) is Self-billed Invoice/credit note (00100000000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSummary and ($isContinuous or $isExport or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))">
          <xsl:attribute name="id">IBR-140-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-140">
<svrl:text>
          ...
          <svrl:text>[IBR-140-OM] - Invoice transaction type (BTOM-001) cannot be Summary invoice (00001000000000000000) if Invoice transaction type (BTOM-001) is Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000), Import of Goods (00000000000010000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isContinuous and ($isSummary or $isDeemed or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))">
          <xsl:attribute name="id">IBR-141-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-141">
<svrl:text>
          ...
          <svrl:text>[IBR-141-OM] - Invoice transaction type (BTOM-001) cannot be Continuous supply (00000100000000000000) if Invoice transaction type (BTOM-001) is Summary invoice (00001000000000000000) or Deemed Supply Invoice (00000001000000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isExport and ($isSelfBilled or $isSummary or $isDeemed or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))">
          <xsl:attribute name="id">IBR-142-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-142">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isDeemed and ($isContinuous or $isExport or $isProfitMargin or $isProfitMarginSelf))">
          <xsl:attribute name="id">IBR-143-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-143">
<svrl:text>
          ...
          <svrl:text>[IBR-143-OM] - Invoice transaction type (BTOM-001) cannot be Deemed Supply Invoice (00000001000000000000) if Invoice transaction type (BTOM-001) is Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isImportRCM and ($isExport or $isProfitMargin or $isProfitMarginSelf or $isImportGoods or $isSelfBilled))">
          <xsl:attribute name="id">IBR-144-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-144">
<svrl:text>
          ...
          <svrl:text>[IBR-144-OM] - Invoice transaction type (BTOM-001) cannot be Import of services for RCM (00000000100000000000) if Invoice transaction type (BTOM-001) is Export Invoice (00000010000000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000) or Self-billed Invoice/credit note (00100000000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isProfitMargin and ($isSummary or $isContinuous or $isExport or $isDeemed or $isImportRCM or $isImportGoods or $isSelfBilled))">
          <xsl:attribute name="id">IBR-145-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-145">
<svrl:text>
          ...
          <svrl:text>[IBR-145-OM] - Invoice transaction type (BTOM-001) cannot be Profit margin invoice (00000000010000000000) if Invoice transaction type (BTOM-001) is Summary invoice (00001000000000000000) or Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Deemed Supply Invoice (00000001000000000000) or Import of services for RCM (00000000100000000000) or Import of Goods (00000000000010000000) or Self-billed Invoice/credit note (00100000000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isProfitMarginSelf and ($isSummary or $isContinuous or $isExport or $isDeemed or $isImportRCM or $isProfitMargin or $isImportGoods or $isSelfBilled))">
          <xsl:attribute name="id">IBR-146-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-146">
<svrl:text>
          ...
          <svrl:text>[IBR-146-OM] - Invoice transaction type (BTOM-001) cannot be Profit Margin Self-Invoice (00000000001000000000) if Invoice transaction type (BTOM-001) Summary invoice (00001000000000000000) or Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Deemed Supply Invoice (00000001000000000000) or Import of services for RCM (00000000100000000000) or Profit margin invoice (00000000010000000000), Import of Goods (00000000000010000000) or Self-billed Invoice/credit note (00100000000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isImportGoods and ($isSummary or $isContinuous or $isExport or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isEcommerce or $isSelfBilled))">
          <xsl:attribute name="id">IBR-147-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-147">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isEcommerce and $isProfitMarginSelf)">
          <xsl:attribute name="id">IBR-148-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-148">
<svrl:text>
          ...
          <svrl:text>[IBR-148-OM] - Invoice transaction type (BTOM-001) cannot be E-commerce supplies (00000000000100000000) if Invoice transaction type (BTOM-001) is Profit Margin Self-Invoice (00000000001000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isSimplified and ($isSelfBilled or $isSummary or $isExport or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSimplified and ($isSelfBilled or $isSummary or $isExport or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))">
          <xsl:attribute name="id">IBR-149-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-149">
<svrl:text>
          ...
          <svrl:text>[IBR-149-OM] - Invoice transaction type (BTOM-001) cannot be Simplified Tax Invoice (01000000000000000000) if Invoice transaction type (BTOM-001) is Self-billed Invoice/credit note (00100000000000000000) or Summary invoice (00001000000000000000) or Export Invoice (00000010000000000000) or Import of services for RCM (00000000100000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSpecialZone) or (cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode and cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode)">
          <xsl:attribute name="id">IBR-150-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-150">
<svrl:text>
          ...
          <svrl:text>[IBR-150-OM] - If Invoice transaction type (BTOM-001) is Special Zone Supplies (00000000000001000000) , buyer country subdivision code (BTOM-026) and Seller country subdivision code (BTOM-024) MUST be provided using the codelist for Country Subdivision (CL-13-OM).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSpecialZone and cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode!='MO') or cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName='SZLN']">
          <xsl:attribute name="id">IBR-151-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-151">
<svrl:text>
          ...
          <svrl:text>[IBR-151-OM] - Seller identifier (IBT-029) is mandatory with Scheme identifier (IBT-029-1) 'SZLN' (Special Zone License Number) if Invoice transaction type (BTOM-001) is Special zone supplies (00000000000001000000) and Seller country subdivision code (BTOM-024) is not equal to 'MO'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<svrl:text>
          ...
          <svrl:text>[IBR-152-OM] - Buyer identifier (IBT-046) is mandatory with Scheme identifier (IBT-046-1) 'SZLN' (Special Zone License Number) if Invoice transaction type (BTOM-001) is Special zone supplies (00000000000001000000) and Buyer country subdivision code (BTOM-026) is not equal to 'MO' except when Buyer electronic address (IBT-049) is '997770000099'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isImportGoods) or cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName='ICID']">
          <xsl:attribute name="id">IBR-153-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-153">
<svrl:text>
          ...
          <svrl:text>[IBR-153-OM] - Buyer identifier (IBT-046) is mandatory with Scheme identifier (IBT-046-1) 'ICID' (Importer Customs ID) if Invoice transaction type (BTOM-001) is Import of Goods (00000000000010000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isExport and $lines[cac:Item/cac:ClassifiedTaxCategory/cbc:TaxExemptionReasonCode ='VATZR-OM-09']) or $lines[cac:Item/cac:ItemSpecificationDocumentReference/cbc:ID[@schemeName='MP']]">
          <xsl:attribute name="id">IBR-155-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-155">
<svrl:text>
          ...
          <svrl:text>[IBR-155-OM] - If invoice transaction type (BTOM-001) is export invoice (00000010000000000000) and at least one VAT exemption reason code (IBT-121) is 'Export of service' then Service Type (BTOM-034) must be provided from the code list for Type of Services (CL-12-OM).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cac:Delivery/cac:Shipment/cac:Delivery/cbc:ActualDeliveryDate)                         or (matches(cac:Delivery/cac:Shipment/cac:Delivery/cbc:ActualDeliveryDate,                                     '^\d{4}-\d{2}-\d{2}$')                             and cac:Delivery/cac:Shipment/cac:Delivery/cbc:ActualDeliveryDate                                 castable as xs:date)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cac:Delivery/cac:Shipment/cac:Delivery/cbc:ActualDeliveryDate) or (matches(cac:Delivery/cac:Shipment/cac:Delivery/cbc:ActualDeliveryDate, '^\d{4}-\d{2}-\d{2}$') and cac:Delivery/cac:Shipment/cac:Delivery/cbc:ActualDeliveryDate castable as xs:date)">
          <xsl:attribute name="id">IBR-156-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-156">
<svrl:text>
          ...
          <svrl:text>[IBR-156-OM] - Import date (BTOM-020) MUST be formatted YYYY-MM-DD and be a valid date.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isImportRCM) or cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode != 'OM'">
          <xsl:attribute name="id">IBR-160-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-160">
<svrl:text>
          ...
          <svrl:text>[IBR-160-OM] - Seller country code (IBT-040) MUST not be'OM' if Invoice transaction type (BTOM-001) is Import of services for RCM (00000000100000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:DocumentDescription)                               or cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:ID = 'OMR'" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:DocumentDescription) or cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:ID = 'OMR'">
          <xsl:attribute name="id">IBR-169-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-169">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cbc:IssueDate)                     or (cbc:IssueDate castable as xs:date                         and adjust-date-to-timezone(xs:date(cbc:IssueDate), ())                             &lt;= adjust-date-to-timezone(                                     xs:date(adjust-dateTime-to-timezone(                                             current-dateTime(),                                             xs:dayTimeDuration('PT4H'))),                                     ()))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cbc:IssueDate) or (cbc:IssueDate castable as xs:date and adjust-date-to-timezone(xs:date(cbc:IssueDate), ()) &lt;= adjust-date-to-timezone( xs:date(adjust-dateTime-to-timezone( current-dateTime(), xs:dayTimeDuration('PT4H'))), ()))">
          <xsl:attribute name="id">IBR-171-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-171">
<svrl:text>
          ...
          <svrl:text>[IBR-171-OM] - Invoice issue date (IBT-002) MUST NOT be a future date (evaluated in Asia/Muscat, UTC+04:00).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($invoiceCurrency='OMR') or not(cac:TaxExchangeRate/cbc:CalculationRate)">
          <xsl:attribute name="id">IBR-172-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-172">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(normalize-space(cac:AccountingCustomerParty/cac:Party/cbc:EndpointID) = '997770000099') or ( normalize-space(cac:AccountingSupplierParty/cbc:AdditionalAccountID) != '' and matches(normalize-space(cac:AccountingSupplierParty/cbc:AdditionalAccountID), '^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[45][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$') )">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-173">
<svrl:text>
            Missing seller UUID.
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isProfitMargin) or exists( cac:BillingReference/cac:InvoiceDocumentReference[ normalize-space(cbc:ID) != '' and normalize-space(cbc:UUID) != '' ] )">
          <xsl:attribute name="id">IBR-175-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-175">
<svrl:text>
          ...
          <svrl:text>[IBR-175-OM] - If Invoice transaction type (BTOM-001) is Profit margin invoice '00000000010000000000', Preceding Invoice reference (IBT-025),  and Preceding invoice UUID (BTOM-031) MUST be present.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isPrepayment and ($isSummary or $isDeemed or $isProfitMarginSelf))">
          <xsl:attribute name="id">IBR-176-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-176">
<svrl:text>
          ...
          <svrl:text>[IBR-176-OM] - Invoice transaction type (BTOM-001) cannot be Prepayment Invoice (00000000000000100000) if Invoice transaction type (BTOM-001) is Summary invoice (00001000000000000000) or Deemed Supply (00000001000000000000) or Profit Margin Self Invoice (00000000001000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($invoiceType=('261','389')) or ($isSelfBilled or $isImportRCM or $isProfitMarginSelf or $isImportGoods)">
          <xsl:attribute name="id">IBR-177-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-177">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cbc:ReceivedDate | cbc:InstallmentDueDate | cbc:DueDate" mode="M50" priority="1010">
    <svrl:fired-rule context="cbc:ReceivedDate | cbc:InstallmentDueDate | cbc:DueDate" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="string-length(text()) = 10 and (string(.) castable as xs:date)">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-171-A">
<svrl:text>
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:TaxTotal/cac:TaxSubtotal | cn:CreditNote/cac:TaxTotal/cac:TaxSubtotal" mode="M50" priority="1008">
    ...
    <xsl:variable name="rate" select="if (cac:TaxCategory/cbc:Percent castable as xs:decimal)                         then xs:decimal(cac:TaxCategory/cbc:Percent)                         else ()" />
    <xsl:variable name="isInvoiceCurrency" select="cbc:TaxAmount/@currencyID = $invoiceCurrency" />
    <xsl:variable name="isTaxCurrency" select="cbc:TaxAmount/@currencyID = $taxCurrency" />
    <xsl:variable name="s09Actual" select="if (cbc:TaxAmount castable as xs:decimal)                         then round(xs:decimal(cbc:TaxAmount) * 100) div 100                         else ()" />
    <xsl:variable name="s09Expected" select="if ($vatCategory = 'S' and exists($rate)) then                         xs:decimal(                             round(                                 (                                     sum($lines[                                             cac:Item/cac:ClassifiedTaxCategory/cbc:ID = $vatCategory                                             and cac:Item/cac:ClassifiedTaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent) = $rate                                         ]/cac:ItemPriceExtension/cac:TaxTotal/cbc:TaxAmount/xs:decimal(.))                                 + sum(for $c in $charges[                                             cac:TaxCategory/cbc:ID = $vatCategory                                             and cac:TaxCategory/cbc:Percent castable as xs:decimal                                             and xs:decimal(cac:TaxCategory/cbc:Percent) = $rate                                         ]                                         return xs:decimal($c/cbc:Amount) * xs:decimal($c/cac:TaxCategory/cbc:Percent) div 100)                                 - sum(for $a in $allowances[                                             cac:TaxCategory/cbc:ID = $vatCategory                                             and cac:TaxCategory/cbc:Percent castable as xs:decimal                                             and xs:decimal(cac:TaxCategory/cbc:Percent) = $rate                                         ]                                         return xs:decimal($a/cbc:Amount) * xs:decimal($a/cac:TaxCategory/cbc:Percent) div 100)                                 ) * 100                             ) div 100                         )                         else ()" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency) or cbc:TaxableAmount" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or cbc:TaxableAmount">
          <xsl:attribute name="id">ALIGNED-IBRP-045</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-045">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or cbc:TaxAmount">
          <xsl:attribute name="id">ALIGNED-IBRP-046</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-046">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or cac:TaxCategory/cbc:ID">
          <xsl:attribute name="id">ALIGNED-IBRP-047</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-047">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency)                          or ($vatCategory = ('E', 'O'))                          or exists(cac:TaxCategory/cbc:Percent)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or ($vatCategory = ('E', 'O')) or exists(cac:TaxCategory/cbc:Percent)">
          <xsl:attribute name="id">ALIGNED-IBRP-048</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-048">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency)                     or $isSimplified                     or not($vatCategory = 'E')                     or (                             cbc:TaxableAmount castable as xs:decimal                             and u:slack(                                 xs:decimal(cbc:TaxableAmount),                                 xs:decimal(round(u:vatBase($vatCategory, $lines, $allowances, $charges) * 100) div 100),                                 $amountTolerance                             )                     )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or $isSimplified or not($vatCategory = 'E') or ( cbc:TaxableAmount castable as xs:decimal and u:slack( xs:decimal(cbc:TaxableAmount), xs:decimal(round(u:vatBase($vatCategory, $lines, $allowances, $charges) * 100) div 100), $amountTolerance ) )">
          <xsl:attribute name="id">ALIGNED-IBRP-E-08-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-E-08">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$isSimplified or not($vatCategory='E') or ( cbc:TaxAmount castable as xs:decimal and xs:decimal(cbc:TaxAmount) = 0 )">
          <xsl:attribute name="id">ALIGNED-IBRP-E-09-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-E-09">
<svrl:text>
            VAT breakdown (IBG-23) – Category 'E' (Exempt)
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency)                     or $isSimplified                     or not($vatCategory = 'O')                     or (                             cbc:TaxableAmount castable as xs:decimal                             and u:slack(                                 xs:decimal(cbc:TaxableAmount),                                 xs:decimal(round(u:vatBase($vatCategory, $lines, $allowances, $charges) * 100) div 100),                                 $amountTolerance                             )                     )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or $isSimplified or not($vatCategory = 'O') or ( cbc:TaxableAmount castable as xs:decimal and u:slack( xs:decimal(cbc:TaxableAmount), xs:decimal(round(u:vatBase($vatCategory, $lines, $allowances, $charges) * 100) div 100), $amountTolerance ) )">
          <xsl:attribute name="id">ALIGNED-IBRP-O-08-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-O-08">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$isSimplified or not($vatCategory='O') or ( cbc:TaxAmount castable as xs:decimal and xs:decimal(cbc:TaxAmount) = 0 )">
          <xsl:attribute name="id">ALIGNED-IBRP-O-09-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-O-09">
<svrl:text>
            VAT breakdown (IBG-23) – Category 'O' (Out of scope)
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency)                     or $isSimplified                     or not($vatCategory = 'Z')                     or (                             cbc:TaxableAmount castable as xs:decimal                             and u:slack(                                 xs:decimal(cbc:TaxableAmount),                                 xs:decimal(round(u:vatBase($vatCategory, $lines, $allowances, $charges) * 100) div 100),                                 $amountTolerance                             )                     )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or $isSimplified or not($vatCategory = 'Z') or ( cbc:TaxableAmount castable as xs:decimal and u:slack( xs:decimal(cbc:TaxableAmount), xs:decimal(round(u:vatBase($vatCategory, $lines, $allowances, $charges) * 100) div 100), $amountTolerance ) )">
          <xsl:attribute name="id">ALIGNED-IBRP-Z-08-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-Z-08">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$isSimplified or not($vatCategory='Z') or ( cbc:TaxAmount castable as xs:decimal and xs:decimal(cbc:TaxAmount) = 0 )">
          <xsl:attribute name="id">ALIGNED-IBRP-Z-09-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-Z-09">
<svrl:text>
            VAT breakdown (IBG-23) – Category 'Z' (Zero rated)
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency) or not($vatCategory = 'S') or (                         cbc:TaxableAmount castable as xs:decimal and                         cac:TaxCategory/cbc:Percent castable as xs:decimal and                         u:slack(                             xs:decimal(cbc:TaxableAmount),                             xs:decimal(round((                                 sum($lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID = $vatCategory and cac:Item/cac:ClassifiedTaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent)]/cbc:LineExtensionAmount/xs:decimal(.)) +                                 sum($charges[cac:TaxCategory/cbc:ID = $vatCategory and xs:decimal(cac:TaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent)]/cbc:Amount/xs:decimal(.)) -                                 sum($allowances[cac:TaxCategory/cbc:ID = $vatCategory and xs:decimal(cac:TaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent)]/cbc:Amount/xs:decimal(.))                             ) * 100) div 100),                             $amountTolerance                         ))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or not($vatCategory = 'S') or ( cbc:TaxableAmount castable as xs:decimal and cac:TaxCategory/cbc:Percent castable as xs:decimal and u:slack( xs:decimal(cbc:TaxableAmount), xs:decimal(round(( sum($lines[cac:Item/cac:ClassifiedTaxCategory/cbc:ID = $vatCategory and cac:Item/cac:ClassifiedTaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent)]/cbc:LineExtensionAmount/xs:decimal(.)) + sum($charges[cac:TaxCategory/cbc:ID = $vatCategory and xs:decimal(cac:TaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent)]/cbc:Amount/xs:decimal(.)) - sum($allowances[cac:TaxCategory/cbc:ID = $vatCategory and xs:decimal(cac:TaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent)]/cbc:Amount/xs:decimal(.)) ) * 100) div 100), $amountTolerance ))">
          <xsl:attribute name="id">ALIGNED-IBRP-S-08-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-S-08">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency)                         or not($vatCategory = 'S')                         or u:slack($s09Actual, $s09Expected, $baisaCap)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or not($vatCategory = 'S') or u:slack($s09Actual, $s09Expected, $baisaCap)">
          <xsl:attribute name="id">ALIGNED-IBRP-S-09-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-S-09">
<svrl:text>
            VAT breakdown (IBG-23) – Category 'S' (Standard rated)
          ...
          <svrl:text>[ALIGNED-IBRP-S-09-OM] - The VAT category tax amount (IBT-117) in a VAT breakdown (IBG-23) where VAT category code (IBT-118) is "S" MUST equal the Σ Invoice-line VAT amounts (BTOM-016) plus charges' VAT minus allowances' VAT, grouped by the VAT category rate (IBT-119) of this breakdown.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT warning-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency)                         or not($vatCategory = 'S')                         or u:slack($s09Actual, $s09Expected, 0)                         or not(u:slack($s09Actual, $s09Expected, $baisaCap))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or not($vatCategory = 'S') or u:slack($s09Actual, $s09Expected, 0) or not(u:slack($s09Actual, $s09Expected, $baisaCap))">
          <xsl:attribute name="id">ALIGNED-IBRP-S-09-OM-WARN</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="role">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-S-09-WARN">
<svrl:text>
            ACCEPTED WITH WARNING — VAT breakdown (IBG-23) Category 'S' tolerance applied.

            Found:
            - VAT amount (IBT-117): '<xsl:text />
              <xsl:value-of select="cbc:TaxAmount" />
              <xsl:text />'
            - Expected VAT amount (2-dec): '<xsl:text />
              <xsl:value-of select="$s09Expected" />
              <xsl:text />'
            - Number of invoice lines: '<xsl:text />
              <xsl:value-of select="$lineCount" />
              <xsl:text />'
            - Tolerance cap (0.001 × lines): '<xsl:text />
              <xsl:value-of select="$baisaCap" />
              <xsl:text />'

            Meaning:
            The VAT category tax amount does not exactly equal the calculated amount, but the
            difference is within the permitted Baisa tolerance (± 0.001 OMR × number of
            invoice lines). The breakdown is ACCEPTED.

            Action:
            No change required for acceptance. Confirm the rounding difference is intentional.
        </svrl:text>
</svrl:diagnostic-reference>
          <svrl:text>[ALIGNED-IBRP-S-09-OM-WARN] - ACCEPTED WITH WARNING: The VAT category tax amount (IBT-117) for category "S" does not exactly equal the sum of invoice-line VAT amounts (BTOM-016) plus charges' VAT minus allowances' VAT, but the difference is within the permitted Baisa tolerance (+/- 0.001 OMR x number of invoice lines). The VAT breakdown is accepted; confirm the rounding is intentional.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='S') or (not(cac:TaxCategory/cbc:TaxExemptionReasonCode) and not(cac:TaxCategory/cbc:TaxExemptionReason))">
          <xsl:attribute name="id">ALIGNED-IBRP-S-10-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-S-10">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($taxCurrency != '' and $isTaxCurrency and $vatCategory = 'S')                         or (                                 $rate castable as xs:decimal                                 and xs:decimal($rate) = $standardVatRate                             )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($taxCurrency != '' and $isTaxCurrency and $vatCategory = 'S') or ( $rate castable as xs:decimal and xs:decimal($rate) = $standardVatRate )">
          <xsl:attribute name="id">IBR-053-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-053">
<svrl:text>
          ...
          <svrl:text>[IBR-053-OM, IBR-104-OM] - If TAX category code for tax category tax amount in accounting currency (IBT-192) is 'S', TAX category rate for tax category tax amount in accounting currency (IBT-193) MUST be 5.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='O') or not(cac:TaxCategory/cbc:Percent)">
          <xsl:attribute name="id">IBR-061-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-061">
<svrl:text>
          ...
          <svrl:text>[IBR-061-OM] - In a VAT breakdown (IBG-23) where VAT category code (IBT-118) is 'Not Subject to VAT', VAT category tax Rate (IBT-119) shall not be provided.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='E') or not(cac:TaxCategory/cbc:Percent)">
          <xsl:attribute name="id">IBR-067-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-067">
<svrl:text>
          ...
          <svrl:text>[IBR-067-OM] - In a VAT breakdown (IBG-23) where VAT category code (IBT-118) is 'Exempt from VAT', VAT category VAT Rate (IBT-119) shall not be provided.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='E' or $vatCategory='Z') or cac:TaxCategory/cbc:TaxExemptionReasonCode">
          <xsl:attribute name="id">IBR-069-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-069">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='O') or not(cac:TaxCategory/cbc:TaxExemptionReasonCode)">
          <xsl:attribute name="id">IBR-070-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-070">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($taxCurrency != '' and $isTaxCurrency and $vatCategory='E') or not(cac:TaxCategory/cbc:Percent)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($taxCurrency != '' and $isTaxCurrency and $vatCategory='E') or not(cac:TaxCategory/cbc:Percent)">
          <xsl:attribute name="id">IBR-095-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-095">
<svrl:text>
          ...
          <svrl:text>[IBR-095-OM, IBR-101-OM] - If TAX category code for tax category tax amount in accounting currency (IBT-192) is 'E', TAX category rate for tax category tax amount in accounting currency (IBT-193) MUST not be present.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($taxCurrency != '' and $isTaxCurrency and $vatCategory='O')                     or not(cac:TaxCategory/cbc:Percent)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($taxCurrency != '' and $isTaxCurrency and $vatCategory='O') or not(cac:TaxCategory/cbc:Percent)">
          <xsl:attribute name="id">IBR-096-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-096">
<svrl:text>
          ...
          <svrl:text>[IBR-096-OM, IBR-102-OM] - If the TAX category code for tax category tax amount in accounting currency (IBT-192) is 'O', then the TAX category rate (IBT-193) MUST NOT be present.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($taxCurrency != '' and $isTaxCurrency and $vatCategory='Z') or (cac:TaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:TaxCategory/cbc:Percent) = 0)">
          <xsl:attribute name="id">IBR-097-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-097">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = 'true'] | cn:CreditNote/cac:AllowanceCharge[cbc:ChargeIndicator = 'true']" mode="M50" priority="1007">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(cbc:BaseAmount and cbc:MultiplierFactorNumeric) or (not(cbc:BaseAmount) and not(cbc:MultiplierFactorNumeric))">
          <xsl:attribute name="id">ALIGNED-IBRP-058</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-058">
<svrl:text>
            [ALIGNED-IBRP-058]
            Found:    BaseAmount=<xsl:text />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="cbc:AllowanceChargeReasonCode">
          <xsl:attribute name="id">IBR-042-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-042">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($cat='S') or (cac:TaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:TaxCategory/cbc:Percent) = $standardVatRate)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat='S') or (cac:TaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:TaxCategory/cbc:Percent) = $standardVatRate)">
          <xsl:attribute name="id">IBR-045-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-045">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat=('E','Z')) or cac:TaxCategory/cbc:TaxExemptionReasonCode">
          <xsl:attribute name="id">IBR-064-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-064">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat='E') or not(cac:TaxCategory/cbc:Percent)">
          <xsl:attribute name="id">IBR-098-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-098">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat='O') or not(cac:TaxCategory/cbc:Percent)">
          <xsl:attribute name="id">IBR-099-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-099">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat='Z') or (cac:TaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:TaxCategory/cbc:Percent) = 0)">
          <xsl:attribute name="id">IBR-100-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-100">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = 'false'] | cn:CreditNote/cac:AllowanceCharge[cbc:ChargeIndicator = 'false']" mode="M50" priority="1006">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(cbc:BaseAmount and cbc:MultiplierFactorNumeric) or (not(cbc:BaseAmount) and not(cbc:MultiplierFactorNumeric))">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-057">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cbc:BaseAmount and cbc:MultiplierFactorNumeric)                         or                         (                             cbc:Amount castable as xs:decimal                             and cbc:BaseAmount castable as xs:decimal                             and cbc:MultiplierFactorNumeric castable as xs:decimal                             and                             u:slack(                                 xs:decimal(cbc:Amount),                                  xs:decimal(                                     round(                                         (                                             xs:decimal(cbc:BaseAmount)                                             * xs:decimal(cbc:MultiplierFactorNumeric)                                             div 100                                         ) * 100                                     ) div 100                                 ),                                  0.01                             )                         )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cbc:BaseAmount and cbc:MultiplierFactorNumeric) or ( cbc:Amount castable as xs:decimal and cbc:BaseAmount castable as xs:decimal and cbc:MultiplierFactorNumeric castable as xs:decimal and u:slack( xs:decimal(cbc:Amount), xs:decimal( round( ( xs:decimal(cbc:BaseAmount) * xs:decimal(cbc:MultiplierFactorNumeric) div 100 ) * 100 ) div 100 ), 0.01 ) )">
          <xsl:attribute name="id">IBR-033-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-033">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat='S') or (cac:TaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:TaxCategory/cbc:Percent) = $standardVatRate)">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-047">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat=('E','Z')) or cac:TaxCategory/cbc:TaxExemptionReasonCode">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-062">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat='E') or not(cac:TaxCategory/cbc:Percent)">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-092">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat='O') or not(cac:TaxCategory/cbc:Percent)">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-093">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($cat='Z') or (cac:TaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:TaxCategory/cbc:Percent) = 0)">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-094">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:AllowanceCharge | cn:CreditNote/cac:AllowanceCharge" mode="M50" priority="1005">
    <svrl:fired-rule context="ubl:Invoice/cac:AllowanceCharge | cn:CreditNote/cac:AllowanceCharge" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cbc:MultiplierFactorNumeric) or cbc:BaseAmount" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cbc:MultiplierFactorNumeric) or cbc:BaseAmount">
          <xsl:attribute name="id">IBR-041-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-041">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(cbc:BaseAmount and cbc:MultiplierFactorNumeric) or ( cbc:Amount castable as xs:decimal and cbc:BaseAmount castable as xs:decimal and cbc:MultiplierFactorNumeric castable as xs:decimal and u:slack( xs:decimal(cbc:Amount), xs:decimal( round( ( xs:decimal(cbc:BaseAmount) * xs:decimal(cbc:MultiplierFactorNumeric) div 100 ) * 100 ) div 100 ), 0.01 ) )">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-063">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:InvoiceLine/cac:AllowanceCharge | cn:CreditNote/cac:CreditNoteLine/cac:AllowanceCharge" mode="M50" priority="1004">
    <svrl:fired-rule context="ubl:Invoice/cac:InvoiceLine/cac:AllowanceCharge | cn:CreditNote/cac:CreditNoteLine/cac:AllowanceCharge" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(cbc:MultiplierFactorNumeric) or cbc:BaseAmount">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-035">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(cbc:BaseAmount and cbc:MultiplierFactorNumeric) or (not(cbc:BaseAmount) and not(cbc:MultiplierFactorNumeric))">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-073">
<svrl:text>
          ...
          <svrl:text>[IBR-073-OM, IBR-074-OM] - Either both or neither Allowance base amount (IBT-137) and percentage (IBT-138), and either both or neither Charge base amount (IBT-142) and percentage (IBT-143) MUST be provided.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:apply-templates mode="M50" select="@*|*" />
	...
	<!--RULE -->
<xsl:template match="cbc:Percent" mode="M50" priority="1003">
    <svrl:fired-rule context="cbc:Percent" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="matches(normalize-space(.), '^\d+(\.\d{1,2})?$') and (if (. castable as xs:decimal) then (xs:decimal(.) >= 0 and xs:decimal(.) &lt;= 100) else false())">
          <xsl:attribute name="id">IBR-046-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-046">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="matches($cleanID, '^OM[0-9]{10}$')">
          <xsl:attribute name="id">IBR-003-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-003">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:ClassifiedTaxCategory | cac:TaxCategory | cac:PartyTaxScheme" mode="M50" priority="1001">
    <svrl:fired-rule context="cac:ClassifiedTaxCategory | cac:TaxCategory | cac:PartyTaxScheme" />
    <xsl:variable name="isSupplier" select="exists(ancestor::cac:AccountingSupplierParty)" />
    <xsl:variable name="isBuyer" select="exists(ancestor::cac:AccountingCustomerParty)" />
    <xsl:variable name="fullXPath" select="string-join(for $node in ancestor-or-self::* return local-name($node), '/')" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="if (exists(ancestor::cac:TaxRepresentativeParty))                             then (cac:TaxScheme/cbc:ID = 'VAT')                         else if (local-name() = 'TaxCategory')                             then (cac:TaxScheme/cbc:ID = 'VAT')                         else if ($isBuyer)                             then (count(../cac:PartyTaxScheme) = 1 and cac:TaxScheme/cbc:ID = 'VAT')                         else if ($isSupplier)                             then (                                 (cac:TaxScheme/cbc:ID = 'VAT' or ../cac:PartyTaxScheme/cac:TaxScheme/cbc:ID = 'VAT') and                                 (if (cac:TaxScheme/cbc:ID = 'VAT') then count(../cac:PartyTaxScheme[cac:TaxScheme/cbc:ID = 'VAT']) = 1 else true())                             )                         else (cac:TaxScheme/cbc:ID = 'VAT')" />
      <xsl:otherwise>
        <svrl:failed-assert test="if (exists(ancestor::cac:TaxRepresentativeParty)) then (cac:TaxScheme/cbc:ID = 'VAT') else if (local-name() = 'TaxCategory') then (cac:TaxScheme/cbc:ID = 'VAT') else if ($isBuyer) then (count(../cac:PartyTaxScheme) = 1 and cac:TaxScheme/cbc:ID = 'VAT') else if ($isSupplier) then ( (cac:TaxScheme/cbc:ID = 'VAT' or ../cac:PartyTaxScheme/cac:TaxScheme/cbc:ID = 'VAT') and (if (cac:TaxScheme/cbc:ID = 'VAT') then count(../cac:PartyTaxScheme[cac:TaxScheme/cbc:ID = 'VAT']) = 1 else true()) ) else (cac:TaxScheme/cbc:ID = 'VAT')">
          <xsl:attribute name="id">IBR-009-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-009">
<svrl:text>
          ...
          <svrl:text>[IBR-009-OM] - Tax scheme code, must be provided in IBT-031-1 or BTOM-006-1 or IBT-048-1 or IBT-063-1 or IBT-095-1 or IBT-102-1 or IBT-118-1 or IBT-167 and shall be 'VAT'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:apply-templates mode="M50" select="@*|*" />
	...
	<!--RULE -->
<xsl:template match="cac:InvoiceLine | cac:CreditNoteLine" mode="M50" priority="1000">
    <svrl:fired-rule context="cac:InvoiceLine | cac:CreditNoteLine" />
    <xsl:variable name="vatCategory" select="normalize-space(cac:Item/cac:ClassifiedTaxCategory/cbc:ID)" />
    <xsl:variable name="vatRate" select="if (cac:Item/cac:ClassifiedTaxCategory/cbc:Percent/normalize-space(.) castable as xs:decimal)                          then xs:decimal(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent)                          else 0" />
    <xsl:variable name="lineTaxAmount" select="cac:ItemPriceExtension/cac:TaxTotal/cbc:TaxAmount" />
    <xsl:variable name="ibr168Actual" select="if ($lineTaxAmount castable as xs:decimal)                         then round(xs:decimal($lineTaxAmount) * 1000) div 1000                         else ()" />
    <xsl:variable name="ibr168Expected" select="if (cbc:LineExtensionAmount castable as xs:decimal and $vatRate castable as xs:decimal)                         then round((xs:decimal(cbc:LineExtensionAmount) * xs:decimal($vatRate) div 100) * 1000) div 1000                         else ()" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($vatCategory='E')                   or not(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='E') or not(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent)">
          <xsl:attribute name="id">ALIGNED-IBRP-E-05-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-E-05">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($vatCategory='O')                   or not(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='O') or not(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent)">
          <xsl:attribute name="id">ALIGNED-IBRP-O-05-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-O-05">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($vatCategory='E')                   or u:isZeroOrEmpty($lineTaxAmount)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='E') or u:isZeroOrEmpty($lineTaxAmount)">
          <xsl:attribute name="id">IBR-039-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-039">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($vatCategory='O')                   or u:isZeroOrEmpty($lineTaxAmount)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='O') or u:isZeroOrEmpty($lineTaxAmount)">
          <xsl:attribute name="id">IBR-054-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-054">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode)                           or cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode/@listID = ('HS','MP')" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode) or cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode/@listID = ('HS','MP')">
          <xsl:attribute name="id">IBR-056-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-056">
<svrl:text>
          ...
          <svrl:text>[IBR-056-OM] - The scheme identifier (IBT-158-1) MUST be 'HS' or 'MP' when Item classification identifier (IBT-158) is provided. Use 'HS' for goods (Oman HS code list) and 'MP' for services (Service Type code list, CL-12-OM).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory = 'Z') or xs:decimal($vatRate) = 0">
          <xsl:attribute name="id">ALIGNED-IBRP-Z-05-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-Z-05">
<svrl:text>
    ...
    <xsl:variable name="lineExtensionAmount" select="if (cbc:LineExtensionAmount/normalize-space(.) castable as xs:decimal)                          then xs:decimal(cbc:LineExtensionAmount)                          else 0" />
    <xsl:variable name="quantity" select="if ((cbc:InvoicedQuantity | cbc:CreditedQuantity)/normalize-space(.) castable as xs:decimal)                          then xs:decimal((cbc:InvoicedQuantity | cbc:CreditedQuantity)[1])                          else 1" />
    <xsl:variable name="priceAmount" select="if (cac:Price/cbc:PriceAmount/normalize-space(.) castable as xs:decimal)                          then xs:decimal(cac:Price/cbc:PriceAmount)                          else 0" />
    <xsl:variable name="baseQuantity" select="                 if (cac:Price/cbc:BaseQuantity/normalize-space(.) castable as xs:decimal                      and xs:decimal(normalize-space(cac:Price/cbc:BaseQuantity)) != 0)                  then xs:decimal(normalize-space(cac:Price/cbc:BaseQuantity))                  else 1" />
    <xsl:variable name="allowancesTotal" select="if (cac:AllowanceCharge[cbc:ChargeIndicator = 'false'])                          then round(sum(                             for $amt in cac:AllowanceCharge[cbc:ChargeIndicator = 'false']/cbc:Amount                             return if ($amt castable as xs:decimal) then xs:decimal($amt) else 0                         ) * 100) div 100                         else 0" />
    <xsl:variable name="chargesTotal" select="if (cac:AllowanceCharge[cbc:ChargeIndicator = 'true'])                          then round(sum(                             for $amt in cac:AllowanceCharge[cbc:ChargeIndicator = 'true']/cbc:Amount                             return if ($amt castable as xs:decimal) then xs:decimal($amt) else 0                         ) * 100) div 100                         else 0" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="u:slack(                         u:to-decimal($lineExtensionAmount),                          u:to-decimal(($quantity * (u:to-decimal($priceAmount) div $baseQuantity)) + $chargesTotal - $allowancesTotal),                          0.01)" />
      <xsl:otherwise>
        <svrl:failed-assert test="u:slack( u:to-decimal($lineExtensionAmount), u:to-decimal(($quantity * (u:to-decimal($priceAmount) div $baseQuantity)) + $chargesTotal - $allowancesTotal), 0.01)">
          <xsl:attribute name="id">IBR-071-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-071">
<svrl:text>
    ...
    <xsl:variable name="grossPrice" select="if (cac:Price/cbc:BaseAmount) then xs:decimal(cac:Price/cbc:BaseAmount) else 0" />
    <xsl:variable name="discountTotal" select="                 if (cac:Price/cac:AllowanceCharge/cbc:Amount)                 then sum(cac:Price/cac:AllowanceCharge/cbc:Amount/xs:decimal(.))                 else 0" />
    <xsl:variable name="netPrice" select="if (cac:Price/cbc:PriceAmount) then xs:decimal(cac:Price/cbc:PriceAmount) else 0" />
    <xsl:variable name="expectedNetPrice" select="                 if ($grossPrice)                 then round(($grossPrice - $discountTotal) * 100) div 100                 else 0" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($grossPrice) or $netPrice = $expectedNetPrice" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($grossPrice) or $netPrice = $expectedNetPrice">
          <xsl:attribute name="id">IBR-075-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-075">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$vatCategory">
          <xsl:attribute name="id">IBR-076-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-076">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($vatCategory='Z')                   or u:isZeroOrEmpty($lineTaxAmount)" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatCategory='Z') or u:isZeroOrEmpty($lineTaxAmount)">
          <xsl:attribute name="id">IBR-077-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-077">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$isSimplified or cac:Item/cac:ItemSpecificationDocumentReference[cbc:ID/@schemeName='MP']/cbc:DocumentTypeCode">
          <xsl:attribute name="id">IBR-078-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-078">
<svrl:text>
          ...
          <svrl:text>[IBR-078-OM] - Goods or Services identification (BTOM-019) must be provided for each item name (IBT-153) except when Invoice transaction type (BTOM-001) is 'Simplified Tax Invoice' (01000000000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$isSimplified or not(cac:Item/cac:ItemSpecificationDocumentReference[cbc:ID/@schemeName='MP']/cbc:DocumentTypeCode='G') or cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS']">
          <xsl:attribute name="id">IBR-079-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-079">
<svrl:text>
          ...
          <svrl:text>[IBR-079-OM] - When Goods or Services identification (BTOM-019) is 'Goods' then Item classification identifier (IBT-158, HS code) must be provided except when Invoice transaction type (BTOM-001) is 'Simplified Tax Invoice' (01000000000000000000)</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS'])                   or string-length(normalize-space(cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS'])) = 12" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS']) or string-length(normalize-space(cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS'])) = 12">
          <xsl:attribute name="id">IBR-080-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-080">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$isSimplified or $isImportGoods or $isImportRCM or $isProfitMarginSelf or cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeName='CC']">
          <xsl:attribute name="id">IBR-081-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-081">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isImportGoods) or cac:Item/cac:OriginCountry/cbc:IdentificationCode">
          <xsl:attribute name="id">IBR-084-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-084">
<svrl:text>
          ...
          <svrl:text>[IBR-084-OM] - If invoice transaction type (BTOM-001) is 'Import of Goods' (00000000000010000000) then Item country of origin (IBT-159) is mandatory.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isProfitMarginSelf)                   or $vatCategory='O'" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isProfitMarginSelf) or $vatCategory='O'">
          <xsl:attribute name="id">IBR-086-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-086">
<svrl:text>
          ...
          <svrl:text>[IBR-086-OM] - If Invoice transaction type (BTOM-001) is 'Profit Margin Self-Invoice ' (00000000001000000000), Invoiced item VAT category code (IBT-151) MUST be 'O' (Not subject to VAT).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isProfitMargin)                   or (every $code in cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS']                       satisfies not(                            starts-with(normalize-space($code),'7101')                         or starts-with(normalize-space($code),'7102')                         or starts-with(normalize-space($code),'7103')                         or starts-with(normalize-space($code),'7104')                         or starts-with(normalize-space($code),'01')                         or starts-with(normalize-space($code),'06')))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isProfitMargin) or (every $code in cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS'] satisfies not( starts-with(normalize-space($code),'7101') or starts-with(normalize-space($code),'7102') or starts-with(normalize-space($code),'7103') or starts-with(normalize-space($code),'7104') or starts-with(normalize-space($code),'01') or starts-with(normalize-space($code),'06')))">
          <xsl:attribute name="id">IBR-091-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-091">
<svrl:text>
          ...
          <svrl:text>[IBR-091-OM] - When Invoice transaction type (BTOM-001) is Profit margin invoice (00000000010000000000), Item classification identifier (IBT-158, listID='HS') MUST NOT start with '7101', '7102', '7103', '7104', '01' or '06'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isSummary) or (substring(cac:InvoicePeriod/cbc:StartDate,1,7) = substring(cac:InvoicePeriod/cbc:EndDate,1,7))">
          ...
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-057">
<svrl:text>
            [IBR-057-OM]
            Context: line-level invoicing period must fall within a single calendar month.
            Found:    Start=<xsl:text />
              <xsl:value-of select="cac:InvoicePeriod/cbc:StartDate" />
              <xsl:text />,
                    End=<xsl:text />
              <xsl:value-of select="cac:InvoicePeriod/cbc:EndDate" />
          ...
          <svrl:text>[IBR-057-OM] - Invoice line period start date (IBT-134) and Invoice line period end date (IBT-135), when provided, must belong to the same calendar month where Invoice transaction type (BTOM-001) is a summary invoice (00001000000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="$isSimplified or $lineTaxAmount">
          <xsl:attribute name="id">IBR-038-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-038">
<svrl:text>
          ...
          <svrl:text>[IBR-038-OM, IBR-068-OM] - An Invoice must contain line Item VAT Amount (BTOM-016) except where invoice transaction type (BTOM-001) is a simplified tax invoice (01000000000000000000).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($isFullTax and $isSummary) or cac:InvoicePeriod/cbc:StartDate and cac:InvoicePeriod/cbc:EndDate">
          <xsl:attribute name="id">IBR-072-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-072">
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cac:Price/cac:AllowanceCharge/cbc:BaseAmount)                     or (                         cac:Price/cbc:PriceAmount castable as xs:decimal and                          cac:Price/cac:AllowanceCharge/cbc:BaseAmount castable as xs:decimal and                         u:slack(                             xs:decimal(cac:Price/cbc:PriceAmount),                             xs:decimal(                                 round(                                     (                                         xs:decimal(cac:Price/cac:AllowanceCharge/cbc:BaseAmount) -                                         sum(cac:Price/cac:AllowanceCharge/cbc:Amount/xs:decimal(.))                                     ) * 100                                 ) div 100                             ),                             0.01                         )                     )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cac:Price/cac:AllowanceCharge/cbc:BaseAmount) or ( cac:Price/cbc:PriceAmount castable as xs:decimal and cac:Price/cac:AllowanceCharge/cbc:BaseAmount castable as xs:decimal and u:slack( xs:decimal(cac:Price/cbc:PriceAmount), xs:decimal( round( ( xs:decimal(cac:Price/cac:AllowanceCharge/cbc:BaseAmount) - sum(cac:Price/cac:AllowanceCharge/cbc:Amount/xs:decimal(.)) ) * 100 ) div 100 ), 0.01 ) )">
          <xsl:attribute name="id">IBR-157-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-157">
<svrl:text>
          ...
          <svrl:text>
                [IBR-157-OM] - Item net price (IBT-146) must be equal to (Item gross price (IBT-148) minus (-) Item price discount (IBT-147)) when item gross price is provided.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="                     not($vatRate)                     or u:slack($ibr168Actual, $ibr168Expected, $baisaTolerance)                 " />
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatRate) or u:slack($ibr168Actual, $ibr168Expected, $baisaTolerance)">
          <xsl:attribute name="id">IBR-168-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-168">
<svrl:text>
            ...
            - VAT amount (BTOM-016): '<xsl:text />
              <xsl:value-of select="cac:ItemPriceExtension/cac:TaxTotal/cbc:TaxAmount" />
              <xsl:text />'.
            - Expected VAT amount (3-dec): '<xsl:text />
              <xsl:value-of select="$ibr168Expected" />
              <xsl:text />'.
            - Per-line tolerance (0.001): '<xsl:text />
              <xsl:value-of select="$baisaTolerance" />
              <xsl:text />'.

            Expected:
            VAT amount = Line net amount × VAT rate ÷ 100.
            A difference up to ± 0.001 OMR per line is accepted (with a warning);
            a difference greater than 0.001 OMR for this line is rejected.

            Fix:
            Recalculate the VAT amount so that BTOM-016 = IBT-131 × IBT-152 ÷ 100
            to within ± 0.001 OMR.
        </svrl:text>
</svrl:diagnostic-reference>
		...
		<!--ASSERT warning-->
<xsl:choose>
      <xsl:when test="                     not($vatRate)                     or u:slack($ibr168Actual, $ibr168Expected, 0)                     or not(u:slack($ibr168Actual, $ibr168Expected, $baisaTolerance))                 " />
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatRate) or u:slack($ibr168Actual, $ibr168Expected, 0) or not(u:slack($ibr168Actual, $ibr168Expected, $baisaTolerance))">
          <xsl:attribute name="id">IBR-168-OM-WARN</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="role">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-168-WARN">
<svrl:text>
            ACCEPTED WITH WARNING — Line VAT amount tolerance applied.
            ...
            Meaning:
            The Line Item VAT amount does not exactly equal the calculation, but the
            difference is within the permitted per-line Baisa tolerance (± 0.001 OMR).
            The invoice line is ACCEPTED.

            Action:
            No change required for acceptance. Confirm the rounding difference is
            intentional.
        </svrl:text>
</svrl:diagnostic-reference>
          <svrl:text>[IBR-168-OM-WARN] - ACCEPTED WITH WARNING: The Line Item VAT amount (BTOM-016) does not exactly equal the calculation (Invoice Line Net Amount (IBT-131) x VAT rate (IBT-152) / 100), but the difference is within the permitted Baisa tolerance (+/- 0.001 OMR per line). The invoice line is accepted; confirm the rounding is intentional.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="$isProfitMargin                         or ($isSimplified and not($lineTaxAmount))                         or (                             cac:ItemPriceExtension/cbc:Amount castable as xs:decimal                             and cbc:LineExtensionAmount castable as xs:decimal                             and u:slack(                                 xs:decimal(cac:ItemPriceExtension/cbc:Amount),                                 xs:decimal(cbc:LineExtensionAmount) + (if ($lineTaxAmount castable as xs:decimal) then xs:decimal($lineTaxAmount) else xs:decimal(0)),                                 $amountTolerance                             )                         )" />
      <xsl:otherwise>
        <svrl:failed-assert test="$isProfitMargin or ($isSimplified and not($lineTaxAmount)) or ( cac:ItemPriceExtension/cbc:Amount castable as xs:decimal and cbc:LineExtensionAmount castable as xs:decimal and u:slack( xs:decimal(cac:ItemPriceExtension/cbc:Amount), xs:decimal(cbc:LineExtensionAmount) + (if ($lineTaxAmount castable as xs:decimal) then xs:decimal($lineTaxAmount) else xs:decimal(0)), $amountTolerance ) )">
          <xsl:attribute name="id">IBR-158-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-158">
<svrl:text>
            ...
            No Baisa tolerance applies to BTOM-017; the value must equal
            IBT-131 + BTOM-016 (within standard 2-decimal rounding).

            Exception:
            This rule does not apply for profit margin invoices.

            Fix:
            Recalculate the total to ensure BTOM-017 = IBT-131 + BTOM-016.
        </svrl:text>
</svrl:diagnostic-reference>
          <svrl:text>[IBR-158-OM] - Total amount including VAT (BTOM-017) must be the sum of Invoice line net amount (IBT-131) and Line Item VAT amount (BTOM-016) unless if Invoice transaction type (BTOM-001) is Profit margin invoice (00000000010000000000) in which case Total amount including VAT (BTOM-017) must be the total sale value of the item.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(cac:Item/cac:ItemSpecificationDocumentReference[cbc:ID/@schemeName='MP']/cbc:DocumentTypeCode='G') or cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS']">
          <xsl:attribute name="id">IBR-174-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-IBR-174">
<svrl:text>
...
<svrl:text>PINT Oman - codelist conformance</svrl:text>

	<!--RULE -->
<xsl:template match="cac:BillingReference/cac:InvoiceDocumentReference/cbc:DocumentStatusCode" mode="M51" priority="1011">
    <svrl:fired-rule context="cac:BillingReference/cac:InvoiceDocumentReference/cbc:DocumentStatusCode" role="fatal" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="((not(contains(normalize-space(.), ' ')) and contains(' CAN VAT VAL QTY OTH ', concat(' ', normalize-space(.), ' '))))" />
      <xsl:otherwise>
        <svrl:failed-assert test="((not(contains(normalize-space(.), ' ')) and contains(' CAN VAT VAL QTY OTH ', concat(' ', normalize-space(.), ' '))))">
          <xsl:attribute name="id">CL-02-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-02-OM">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cbc:InvoiceTypeCode[@name] | cbc:CreditNoteTypeCode[@name]" mode="M51" priority="1010">
    <svrl:fired-rule context="cbc:InvoiceTypeCode[@name] | cbc:CreditNoteTypeCode[@name]" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(substring($t,1,1)='1' and substring($t,2,1)='1')">
          <xsl:attribute name="id">CL-03-OM-1</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-03-OM">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="substring($t,1,1)='1' or substring($t,2,1)='1'">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:TaxCategory/cbc:ID | cac:ClassifiedTaxCategory/cbc:ID" mode="M51" priority="1009">
    <svrl:fired-rule context="cac:TaxCategory/cbc:ID | cac:ClassifiedTaxCategory/cbc:ID" role="fatal" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="((not(contains(normalize-space(.), ' ')) and contains(' S E O Z ', concat(' ', normalize-space(.), ' '))))" />
      <xsl:otherwise>
        <svrl:failed-assert test="((not(contains(normalize-space(.), ' ')) and contains(' S E O Z ', concat(' ', normalize-space(.), ' '))))">
          <xsl:attribute name="id">CL-04-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-04-OM">
<svrl:text>
            VAT category code
	...
	<!--RULE -->
<xsl:template match="cac:TaxCategory/cbc:TaxExemptionReasonCode | cac:ClassifiedTaxCategory/cbc:TaxExemptionReasonCode" mode="M51" priority="1008">
    <svrl:fired-rule context="cac:TaxCategory/cbc:TaxExemptionReasonCode | cac:ClassifiedTaxCategory/cbc:TaxExemptionReasonCode" role="fatal" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($catCode = 'E') or (not(contains($reason, ' ')) and contains(' VATEX-OM-01 VATEX-OM-02 VATEX-OM-03 VATEX-OM-04 VATEX-OM-05 VATEX-OM-06 VATEX-OM-07 VATEX-OM-08 VATEX-OM-09 VATEX-OM-10 VATEX-OM-11 VATEX-OM-12 ', concat(' ', $reason, ' ')))">
          <xsl:attribute name="id">CL-05-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-05-OM">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not($catCode = 'Z') or (not(contains($reason, ' ')) and contains(' VATZR-OM-01 VATZR-OM-02 VATZR-OM-03 VATZR-OM-04 VATZR-OM-05 VATZR-OM-06 VATZR-OM-07 VATZR-OM-08 VATZR-OM-09 VATZR-OM-10 VATZR-OM-11 VATZR-OM-12 VATZR-OM-13 VATZR-OM-14 VATZR-OM-15 VATZR-OM-16 ', concat(' ', $reason, ' ')))">
          <xsl:attribute name="id">CL-10-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-10-OM">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName] | cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName]" mode="M51" priority="1007">
    <svrl:fired-rule context="cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName] | cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName]" role="fatal" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="((not(contains(normalize-space(@schemeName), ' ')) and contains(' CR TIN CID PASNUM OTHID ICID SZLN ', concat(' ', normalize-space(@schemeName), ' '))))">
          <xsl:attribute name="id">CL-06-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-06-OM">
<svrl:text>
            Buyer/Seller identifier scheme (IBT-029-1 / IBT-046-1)

            Found:
            - Value: '<xsl:text />
              <xsl:value-of select="normalize-space(@schemeName)" />
              <xsl:text />'
	...
	<!--RULE -->
<xsl:template match="cac:Item/cac:ItemSpecificationDocumentReference[cbc:ID/@schemeName='MP']/cbc:DocumentTypeCode" mode="M51" priority="1006">
    <svrl:fired-rule context="cac:Item/cac:ItemSpecificationDocumentReference[cbc:ID/@schemeName='MP']/cbc:DocumentTypeCode" role="fatal" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="((not(contains(normalize-space(.), ' ')) and contains(' G S ', concat(' ', normalize-space(.), ' '))))" />
      <xsl:otherwise>
        <svrl:failed-assert test="((not(contains(normalize-space(.), ' ')) and contains(' G S ', concat(' ', normalize-space(.), ' '))))">
          <xsl:attribute name="id">CL-07-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-07-OM">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="if (string-length($hsCode) = 0) then true() else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '04') then contains($codes01, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '15') then contains($codes02, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '21') then contains($codes03, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '26') then contains($codes04, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '28') then contains($codes05, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '29') then contains($codes06, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '30') then contains($codes07, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '38') then contains($codes08, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '41') then contains($codes09, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '48') then contains($codes10, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '55') then contains($codes11, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '62') then contains($codes12, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '70') then contains($codes13, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '73') then contains($codes14, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '83') then contains($codes15, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '84') then contains($codes16, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '85') then contains($codes17, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '87') then contains($codes18, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '90') then contains($codes19, concat(' ', $hsCode, ' ')) else if (string-length($hsCode) = 12 and substring($hsCode, 1, 2) &lt;= '98') then contains($codes20, concat(' ', $hsCode, ' ')) else false()">
          <xsl:attribute name="id">CL-08-OM-HS</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-08-OM-HS">
<svrl:text>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="if (string-length($isicCode) = 0) then true() else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '0') then contains($isic0, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '1') then contains($isic1, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '2') then contains($isic2, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '3') then contains($isic3, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '4') then contains($isic4, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '5') then contains($isic5, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '6') then contains($isic6, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '7') then contains($isic7, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '8') then contains($isic8, concat(' ', $isicCode, ' ')) else if (string-length($isicCode) = 6 and substring($isicCode, 1, 1) = '9') then contains($isic9, concat(' ', $isicCode, ' ')) else false()">
          <xsl:attribute name="id">CL-08-OM-ISIC</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-08-OM-ISIC">
<svrl:text>
            The supplied value '<xsl:text />
	...
	<!--RULE -->
<xsl:template match="cac:Delivery/cac:DeliveryTerms/cbc:ID" mode="M51" priority="1003">
    <svrl:fired-rule context="cac:Delivery/cac:DeliveryTerms/cbc:ID" role="fatal" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="((not(contains(normalize-space(.), ' ')) and contains(' EXW FCA CPT CIP DAP DPU DDP FAS FOB CFR CIF ', concat(' ', normalize-space(.), ' '))))">
          <xsl:attribute name="id">CL-09-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-09-OM">
<svrl:text>
            Incoterms (BTOM-022)
	...
	<!--RULE -->
<xsl:template match="cac:AdditionalItemProperty/cbc:NameCode" mode="M51" priority="1002">
    <svrl:fired-rule context="cac:AdditionalItemProperty/cbc:NameCode" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(not($isProfitMargin or $isProfitMarginSelf)) or ( normalize-space(.) != '' and not(contains(normalize-space(.), ' ')) and contains( ' VATPM-OM-01 VATPM-OM-02 VATPM-OM-03 VATPM-OM-04 VATPM-OM-05 ', concat(' ', normalize-space(.), ' ') ) )">
          <xsl:attribute name="id">CL-11-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-11-OM">
<svrl:text>
          ...
          <svrl:text>[CL-11-OM] - If invoice transaction type (BTOM-001) is 'Profit Margin Self-Invoice' (00000000001000000000) or Profit margin invoice (00000000010000000000), Profit margin item reason code (BTOM-025) MUST be present and coded using Profit Margin Items Codelist (CL-11-OM).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:apply-templates mode="M51" select="@*|*" />
	...
	<!--RULE -->
<xsl:template match="cac:Item/cac:ItemSpecificationDocumentReference/cbc:ID[@schemeName='MP']" mode="M51" priority="1001">
    <svrl:fired-rule context="cac:Item/cac:ItemSpecificationDocumentReference/cbc:ID[@schemeName='MP']" role="fatal" />
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="((not(contains(normalize-space(.), ' ')) and contains(' 64000000 70000000 71000000 72000000 73000000 76000000 77000000 78000000 80000000 81000000 82000000 83000000 84000000 85000000 86000000 90000000 91000000 92000000 93000000 94000000 00000000 ', concat(' ', normalize-space(.), ' '))))">
          <xsl:attribute name="id">CL-12-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-12-OM">
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode | cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode" mode="M51" priority="1000">
    <svrl:fired-rule context="cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode | cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode" role="fatal" />

		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="((not(contains(normalize-space(.), ' ')) and contains(' SHRFZ SEZAD SLLFZ AFZ MO OTH ', concat(' ', normalize-space(.), ' '))))" />
      <xsl:otherwise>
        <svrl:failed-assert test="((not(contains(normalize-space(.), ' ')) and contains(' SHRFZ SEZAD SLLFZ AFZ MO OTH ', concat(' ', normalize-space(.), ' '))))">
          <xsl:attribute name="id">CL-13-OM</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="role">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute> <svrl:diagnostic-reference diagnostic="d-CL-13-OM">
<svrl:text>
