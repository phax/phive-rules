...
<xsl:function as="xs:decimal" name="u:vatBase">
  ...
  <xsl:function as="xs:boolean" name="u:hasValidDecimal">
        <xsl:param as="node()*" name="n" />
...
<svrl:text>PINT Oman E-Invoice Validation Rules (IBR-OM)</svrl:text>
  ...
  <xsl:param name="isValidBitString" select="matches($txnType, '^[X1]{20}$')" />
...
<svrl:text>PINT Oman - business rules and calculation consistency</svrl:text>
	...
	<!--RULE -->
<xsl:template match="/" mode="M51" priority="1012">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice | cn:CreditNote" mode="M51" priority="1011">
...
<xsl:if test="$debugMode">
      <svrl:successful-report test="$debugMode">
        <xsl:attribute name="role">information</xsl:attribute>
        <xsl:attribute name="location">
          <xsl:apply-templates mode="schematron-select-full-path" select="." />
        </xsl:attribute>
        <svrl:text>
          ...
          <xsl:text />
                SumLines:       <xsl:text />
          <xsl:value-of select="sum($lines/cbc:LineExtensionAmount/xs:decimal(.))" />
          <xsl:text />
                SumAllowances:  <xsl:text />
          <xsl:value-of select="sum($allowances/cbc:Amount/xs:decimal(.))" />
          <xsl:text />
                SumCharges:     <xsl:text />
          <xsl:value-of select="sum($charges/cbc:Amount/xs:decimal(.))" />
          <xsl:text />
                TaxExclusive:   <xsl:text />
          <xsl:value-of select="cac:LegalMonetaryTotal/cbc:TaxExclusiveAmount" />
          <xsl:text />
                TaxInclusive:   <xsl:text />
          ...
          <xsl:text />
                Subtotals:      <xsl:text />
          <xsl:value-of select="                                     string-join(                                         for $c in ('S','Z','E','O')                                         return concat($c, '=', count(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cbc:ID=$c])),                                         ' / ')" />
          <xsl:text />
            </svrl:text>
      </svrl:successful-report>
    </xsl:if>
...
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="string-length($txnType) = 20 and matches($txnType, '^[X1]{20}$')" />
      <xsl:otherwise>
        <svrl:failed-assert test="string-length($txnType) = 20 and matches($txnType, '^[X1]{20}$')">
...
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="matches(normalize-space(cbc:UUID),                   '^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$')" />
      <xsl:otherwise>
        <svrl:failed-assert test="matches(normalize-space(cbc:UUID), '^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$')">
...
<svrl:text>
          ...
          <svrl:text>[IBR-002-OM] - The unique identifier (“UUID”) (BTOM-002) must be provided on the invoice and must contain only letters, digits, and dashes.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<svrl:text>
          ...
          <svrl:text>[IBR-016-OM] - Either Buyer identifier (IBT-046) or Buyer VATIN (IBT-048) MUST be present when the Invoice transaction type (BTOM-001) is a Full tax invoice (1XXXXXXXXXXXXXXXXXXX) OR Third-party Invoice (XXX1XXXXXXXXXXXXXXXX) OR Summary invoice (XXXX1XXXXXXXXXXXXXXX) OR Continuous supply (XXXXX1XXXXXXXXXXXXXX) OR Export Invoice (XXXXXX1XXXXXXXXXXXXX) OR Profit margin invoice (XXXXXXXXX1XXXXXXXXXX) OR E-commerce supplies (XXXXXXXXXXX1XXXXXXXX).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(exists(cac:LegalMonetaryTotal/cbc:PrepaidAmount)) or (exists(cac:OriginatorDocumentReference/cbc:ID) and exists(cac:OriginatorDocumentReference/cbc:UUID))">
...
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not(cbc:IssueDate)                     or (cbc:IssueDate castable as xs:date                         and xs:date(cbc:IssueDate)                             &lt;= xs:date(adjust-dateTime-to-timezone(current-dateTime(), xs:dayTimeDuration('PT4H'))))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(cbc:IssueDate) or (cbc:IssueDate castable as xs:date and xs:date(cbc:IssueDate) &lt;= xs:date(adjust-dateTime-to-timezone(current-dateTime(), xs:dayTimeDuration('PT4H'))))">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(normalize-space(cac:AccountingCustomerParty/cac:Party/cbc:EndpointID) = '997770000099') or ( normalize-space(cac:AccountingSupplierParty/cbc:AdditionalAccountID) != '' and matches(normalize-space(cac:AccountingSupplierParty/cbc:AdditionalAccountID), '^[A-Za-z0-9-]+$') )">
...
<svrl:text>
          ...
          <svrl:text>[IBR-175-OM] - If Invoice transaction type (BTOM-001) is Profit margin invoice 'XXXXXXXXX1XXXXXXXXXX', Preceding Invoice reference (IBT-025),  and Preceding invoice UUID (BTOM-031) MUST be present.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<svrl:text>
          ...
          <svrl:text>[IBR-176-OM] - Invoice transaction type (BTOM-001) cannot be Prepayment Invoice (XXXXXXXXXXXXXX1XXXXX) if Invoice transaction type (BTOM-001) is Summary invoice (XXXX1XXXXXXXXXXXXXXX) or Deemed Supply (XXXXXXX1XXXXXXXXXXXX) or Profit Margin Self Invoice (XXXXXXXXXX1XXXXXXXXX).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cbc:ReceivedDate | cbc:InstallmentDueDate" mode="M51" priority="1010">
    <svrl:fired-rule context="cbc:ReceivedDate | cbc:InstallmentDueDate" />
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:TaxTotal/cac:TaxSubtotal | cn:CreditNote/cac:TaxTotal/cac:TaxSubtotal" mode="M51" priority="1008">
...
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="not($isInvoiceCurrency) or not($vatCategory = 'S') or (                             cbc:TaxAmount castable as xs:decimal                             and u:slack(                                 xs:decimal(cbc:TaxAmount),                                 xs:decimal(                                     round(                                         (                                             sum($lines[                                                     cac:Item/cac:ClassifiedTaxCategory/cbc:ID = $vatCategory                                                     and xs:decimal(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent)                                                         = xs:decimal(current()/cac:TaxCategory/cbc:Percent)                                                 ]/cac:ItemPriceExtension/cac:TaxTotal/cbc:TaxAmount/xs:decimal(.))                                         + sum(for $c in $charges[                                                     cac:TaxCategory/cbc:ID = $vatCategory                                                     and cac:TaxCategory/cbc:Percent castable as xs:decimal                                                     and xs:decimal(cac:TaxCategory/cbc:Percent)                                                         = xs:decimal(current()/cac:TaxCategory/cbc:Percent)                                                 ]                                                 return xs:decimal($c/cbc:Amount)                                                     * xs:decimal($c/cac:TaxCategory/cbc:Percent) div 100)                                         - sum(for $a in $allowances[                                                     cac:TaxCategory/cbc:ID = $vatCategory                                                     and cac:TaxCategory/cbc:Percent castable as xs:decimal                                                     and xs:decimal(cac:TaxCategory/cbc:Percent)                                                         = xs:decimal(current()/cac:TaxCategory/cbc:Percent)                                                 ]                                                 return xs:decimal($a/cbc:Amount)                                                     * xs:decimal($a/cac:TaxCategory/cbc:Percent) div 100)                                         ) * 100                                     ) div 100                                 ),                                 $amountTolerance                             )                         )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not($isInvoiceCurrency) or not($vatCategory = 'S') or ( cbc:TaxAmount castable as xs:decimal and u:slack( xs:decimal(cbc:TaxAmount), xs:decimal( round( ( sum($lines[ cac:Item/cac:ClassifiedTaxCategory/cbc:ID = $vatCategory and xs:decimal(cac:Item/cac:ClassifiedTaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent) ]/cac:ItemPriceExtension/cac:TaxTotal/cbc:TaxAmount/xs:decimal(.)) + sum(for $c in $charges[ cac:TaxCategory/cbc:ID = $vatCategory and cac:TaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:TaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent) ] return xs:decimal($c/cbc:Amount) * xs:decimal($c/cac:TaxCategory/cbc:Percent) div 100) - sum(for $a in $allowances[ cac:TaxCategory/cbc:ID = $vatCategory and cac:TaxCategory/cbc:Percent castable as xs:decimal and xs:decimal(cac:TaxCategory/cbc:Percent) = xs:decimal(current()/cac:TaxCategory/cbc:Percent) ] return xs:decimal($a/cbc:Amount) * xs:decimal($a/cac:TaxCategory/cbc:Percent) div 100) ) * 100 ) div 100 ), $amountTolerance ) )">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = 'true'] | cn:CreditNote/cac:AllowanceCharge[cbc:ChargeIndicator = 'true']" mode="M51" priority="1007">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:AllowanceCharge[cbc:ChargeIndicator = 'false'] | cn:CreditNote/cac:AllowanceCharge[cbc:ChargeIndicator = 'false']" mode="M51" priority="1006">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:AllowanceCharge | cn:CreditNote/cac:AllowanceCharge" mode="M51" priority="1005">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:InvoiceLine/cac:AllowanceCharge | cn:CreditNote/cac:CreditNoteLine/cac:AllowanceCharge" mode="M51" priority="1004">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cbc:Percent" mode="M51" priority="1003">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:ClassifiedTaxCategory | cac:TaxCategory | cac:PartyTaxScheme" mode="M51" priority="1001">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="if (local-name() = 'TaxCategory') then (cac:TaxScheme/cbc:ID = 'VAT') else if ($isBuyer) then (count(../cac:PartyTaxScheme) = 1 and cac:TaxScheme/cbc:ID = 'VAT') else if ($isSupplier) then ( (cac:TaxScheme/cbc:ID = 'VAT' or ../cac:PartyTaxScheme/cac:TaxScheme/cbc:ID = 'VAT') and (if (cac:TaxScheme/cbc:ID = 'VAT') then count(../cac:PartyTaxScheme[cac:TaxScheme/cbc:ID = 'VAT']) = 1 else true()) ) else (cac:TaxScheme/cbc:ID = 'VAT')">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:InvoiceLine | cac:CreditNoteLine" mode="M51" priority="1000">
...
<svrl:text>
          ...
          <svrl:text>[IBR-038-OM, IBR-068-OM] - An Invoice must contain line Item VAT Amount (BTOM-016) except where invoice transaction type (BTOM-001) is a simplified tax invoice (X1XXXXXXXXXXXXXXXXXX).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="                     not($vatRate)                     or                     (                         $lineTaxAmount castable as xs:decimal                         and cbc:LineExtensionAmount castable as xs:decimal                         and $vatRate castable as xs:decimal                         and                         u:slack(                             xs:decimal($lineTaxAmount),                             round(                                 (                                     xs:decimal(cbc:LineExtensionAmount)                                     * xs:decimal($vatRate)                                     div 100                                 ) * 100                             ) div 100,                             0.01                         )                     )                 " />
      <xsl:otherwise>
        <svrl:failed-assert test="not($vatRate) or ( $lineTaxAmount castable as xs:decimal and cbc:LineExtensionAmount castable as xs:decimal and $vatRate castable as xs:decimal and u:slack( xs:decimal($lineTaxAmount), round( ( xs:decimal(cbc:LineExtensionAmount) * xs:decimal($vatRate) div 100 ) * 100 ) div 100, 0.01 ) )">
...
<svrl:text>
		...
		<!--ASSERT fatal-->
<xsl:choose>
      <xsl:when test="$isProfitMargin                         or (                             cac:ItemPriceExtension/cbc:Amount castable as xs:decimal                             and u:slack(                                 xs:decimal(cac:ItemPriceExtension/cbc:Amount),                                 xs:decimal(xs:decimal(cbc:LineExtensionAmount) + xs:decimal($lineTaxAmount)),                                 0.01                             )                         )" />
      <xsl:otherwise>
        <svrl:failed-assert test="$isProfitMargin or ( cac:ItemPriceExtension/cbc:Amount castable as xs:decimal and u:slack( xs:decimal(cac:ItemPriceExtension/cbc:Amount), xs:decimal(xs:decimal(cbc:LineExtensionAmount) + xs:decimal($lineTaxAmount)), 0.01 ) )">
...
<svrl:text>PINT Oman - codelist conformance</svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:BillingReference/cac:InvoiceDocumentReference/cbc:DocumentStatusCode" mode="M52" priority="1011">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cbc:InvoiceTypeCode[@name] | cbc:CreditNoteTypeCode[@name]" mode="M52" priority="1010">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:TaxCategory/cbc:ID | cac:ClassifiedTaxCategory/cbc:ID" mode="M52" priority="1009">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:TaxCategory/cbc:TaxExemptionReasonCode | cac:ClassifiedTaxCategory/cbc:TaxExemptionReasonCode" mode="M52" priority="1008">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName] | cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName]" mode="M52" priority="1007">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:Item/cac:ItemSpecificationDocumentReference[cbc:ID/@schemeName='MP']/cbc:DocumentTypeCode" mode="M52" priority="1006">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS']" mode="M52" priority="1005">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeName='CC']" mode="M52" priority="1004">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:Delivery/cac:DeliveryTerms/cbc:ID" mode="M52" priority="1003">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:AdditionalItemProperty/cbc:NameCode" mode="M52" priority="1002">
...
<svrl:text>
          ...
          <svrl:text>[CL-11-OM] - If invoice transaction type (BTOM-001) is 'Profit Margin Self-Invoice' (XXXXXXXXXX1XXXXXXXXX) or Profit margin invoice (XXXXXXXXX1XXXXXXXXXX), Profit margin item reason code (BTOM-025) MUST be present and coded using Profit Margin Items Codelist (CL-11-OM).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:apply-templates mode="M52" select="@*|*" />
	...
	<!--RULE -->
<xsl:template match="cac:Item/cac:ItemSpecificationDocumentReference/cbc:ID[@schemeName='MP']" mode="M52" priority="1001">
...
<svrl:text>
	...
	<!--RULE -->
<xsl:template match="cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode | cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode" mode="M52" priority="1000">
