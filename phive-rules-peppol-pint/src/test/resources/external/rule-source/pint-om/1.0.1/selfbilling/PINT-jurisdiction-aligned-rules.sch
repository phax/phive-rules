<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" queryBinding="xslt2">
   ...
   <xsl:function name="u:to-decimal" as="xs:decimal">
    ...
    <let name="baisaTolerance" value="xs:decimal('0.001')"/>
    <let name="lineCount" value="count($lines)"/>
    <let name="baisaCap" value="$baisaTolerance * $lineCount"/>
    ...
    <pattern id="Aligned-om-rules">
        ...
        <rule context="ubl:Invoice | cn:CreditNote">
            ...
            <assert id="ALIGNED-IBRP-001-OM" flag="fatal" role="fatal" test="starts-with($customizationID, 'urn:peppol:pint:selfbilling-1@om-1')" diagnostics="d-001">[ALIGNED-IBRP-001-OM] - Specification identifier (IBT-024) MUST start with the value 'urn:peppol:pint:selfbilling-1@om-1'.</assert>

            
            
            <assert id="ALIGNED-IBRP-002-OM" flag="fatal" role="fatal" test="starts-with($profileID, 'urn:peppol:bis:selfbilling')" diagnostics="d-002">[ALIGNED-IBRP-002-OM] - Business process (IBT-023) must follow the expected format 'urn:peppol:bis:selfbilling'.</assert>
            ...
            <assert id="IBR-001-OM" flag="fatal" role="fatal" test="string-length($txnType) = 20 and matches($txnType, '^[01]{20}$')" diagnostics="d-IBR-001">[IBR-001-OM] - Invoice transaction type (BTOM-001) must be a 20-character string consisting only of '1' and '0'.</assert>
            <assert id="IBR-002-OM" flag="fatal" role="fatal" test="matches(normalize-space(cbc:UUID),                   '^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-5[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$')" diagnostics="d-IBR-002">[IBR-002-OM] - The unique identifier (“UUID”) (BTOM-002) must be provided on the invoice and MUST be a valid UUID version 5.</assert>
            ...
            <assert id="IBR-012-OM" flag="fatal" role="fatal" test="not($isExport and $lines//cbc:TaxExemptionReasonCode = 'VATZR-OM-09')                   or cac:Delivery/cac:DeliveryLocation/cac:Address/cac:Country/cbc:IdentificationCode != 'OM'" diagnostics="d-IBR-012">[IBR-012-OM] - Deliver to country code (IBT-080) must not be 'OM' if invoice transaction type (BTOM-001) is export invoice (00000010000000000000) and at least one VAT exemption reason code (IBT-121) is 'Export of service (VATZR-OM-09)'.</assert>
            
            <assert id="IBR-013-OM" flag="fatal" role="fatal" test="not($isExport and $lines//cbc:TaxExemptionReasonCode ='VATZR-OM-12')                          or (exists($doc/cac:AdditionalDocumentReference                             [not(cbc:DocumentTypeCode='130')]                             [cbc:ID and cbc:UUID]))" diagnostics="d-IBR-013">[IBR-013-OM] - Supporting document reference (IBT-122) and Supporting document UUID (BTOM-023) must be provided if invoice transaction type (BTOM-001) is export invoice (00000010000000000000) and at least one VAT exemption reason code (IBT-121) is 'Re-export of goods (VATZR-OM-12)'.</assert>
            
            <assert id="IBR-014-OM" flag="fatal" role="fatal" test="not($isExport)                   or cac:Delivery/cac:DeliveryLocation/cac:Address/cac:Country/cbc:IdentificationCode" diagnostics="d-IBR-014">[IBR-014-OM] - Deliver to country code (IBT-080) must be provided if invoice transaction type (BTOM-001) is export invoice (00000010000000000000).</assert>
            ...
            <assert id="IBR-036-OM" flag="fatal" role="fatal" test="not($isSummary)                   or (substring(cac:InvoicePeriod/cbc:StartDate,1,7)                       = substring(cac:InvoicePeriod/cbc:EndDate,1,7))" diagnostics="d-IBR-036">[IBR-036-OM] - Invoicing period Start date (IBT-073) and Invoicing period end date (IBT-074) provided must belong to the same calendar month where Invoice transaction type code (BTOM-001) is a summary invoice (00001000000000000000).</assert>
            
            <assert id="IBR-037-OM" flag="fatal" role="fatal" test="not($isSummary or $isContinuous)                   or (cac:InvoicePeriod/cbc:StartDate and cac:InvoicePeriod/cbc:EndDate)" diagnostics="d-IBR-037">[IBR-037-OM] - Invoicing period start date (IBT-073) and the Invoicing period end date (IBT-074) must be provided where Invoice transaction type code (BTOM-001) is a summary invoice (00001000000000000000) or Continuous supply (00000100000000000000).</assert>
            
            <assert id="IBR-040-OM" flag="fatal" role="fatal" test="not($isEcommerce)                   or (cac:Delivery/cac:DeliveryLocation/cac:Address/cbc:StreetName                       and cac:Delivery/cac:DeliveryLocation/cac:Address/cbc:AdditionalStreetName                       and cac:Delivery/cac:DeliveryLocation/cac:Address/cbc:CityName                       and cac:Delivery/cac:DeliveryLocation/cac:Address/cbc:PostalZone                       and cac:Delivery/cac:DeliveryLocation/cac:Address/cac:Country/cbc:IdentificationCode                        and cac:Delivery/cac:DeliveryLocation/cac:Address/cac:AddressLine/cbc:Line)" diagnostics="d-IBR-040">[IBR-040-OM] - Deliver to address line 1 - Postal code (IBT-075), Deliver to address line 2 - Postal code area (IBT-076), Deliver to address line 3 - Area (IBT-165), Deliver to city (IBT-077), Deliver to post code - PO Box(IBT-078), Deliver to country code (IBT-080) MUST be present when the Invoice transaction type (BTOM-001) is E-commerce supplies (00000000000100000000).</assert>
            ...
            <assert id="IBR-058-OM" flag="fatal" role="fatal" test="not(exists(cac:LegalMonetaryTotal/cbc:PrepaidAmount))                         or (exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:ID)                             and exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:UUID))" diagnostics="d-IBR-058">[IBR-058-OM] - Prepayment invoice number (BTOM-027) and Prepayment invoice UUID (BTOM-014) must be provided if Paid amount (IBT-180) is present.</assert>
            ...
            <assert id="IBR-082-OM" flag="fatal" role="fatal" test="not($isProfitMargin or $isProfitMarginSelf)                         or (                             cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:DocumentDescription castable as xs:decimal                             and u:slack(                                 xs:decimal(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PM_TOTAL']/cbc:DocumentDescription),                                 xs:decimal(sum($lines/cac:ItemPriceExtension/cbc:Amount/xs:decimal(.))),                                 0.01                             )                         )" diagnostics="d-IBR-082">[IBR-082-OM] - When Invoice transaction type (BTOM-001) is Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000), then Total Amount Due (BTOM-020), should be provided and is mandatory and must be the sum of Total amount including VAT (BTOM-017).</assert>
            
            <assert id="IBR-085-OM" flag="fatal" role="fatal" test="                 not($isImportGoods)                 or                 (                     cac:Delivery/cac:Shipment/cac:Delivery/cbc:ActualDeliveryDate                     and cac:Delivery/cac:Shipment/cbc:ID                     and cac:Delivery/cac:DeliveryTerms/cbc:ID                 )                 " diagnostics="d-IBR-085">[IBR-085-OM] - If invoice transaction type (BTOM-001) is 'Import of Goods' (00000000000010000000), then Import date (BTOM-030), Custom Declaration number (BTOM-021) and Incoterms (BTOM-022).MUST be present.</assert>
            
            
            <assert id="IBR-087-OM" flag="fatal" role="fatal" test="not($isProfitMarginSelf)                 or cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode='OM'" diagnostics="d-IBR-087">[IBR-087-OM] - In case Invoice transaction type (BTOM-001) is 'Profit Margin Self-Invoice' (00000000001000000000), Seller Country Code (IBT-040) MUST be 'OM'.</assert>
            ...
            <assert id="IBR-138-OM" flag="fatal" role="fatal" test="not($isSelfBilled and ($isThirdParty or $isExport or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))" diagnostics="d-IBR-138">[IBR-138-OM] - Invoice transaction type (BTOM-001) cannot be Self-billed Invoice/credit note (00100000000000000000) if Invoice transaction type (BTOM-001) is Third-party Invoice (00010000000000000000) or Export Invoice (00000010000000000000) or Import of services for RCM (00000000100000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000).</assert>
            
            <assert id="IBR-139-OM" flag="fatal" role="fatal" test="not($isThirdParty and $isSelfBilled)" diagnostics="d-IBR-139">[IBR-139-OM] - Invoice transaction type (BTOM-001) cannot be Third-party Invoice (00010000000000000000) if Invoice transaction type (BTOM-001) is Self-billed Invoice/credit note (00100000000000000000).</assert>
            
            <assert id="IBR-140-OM" flag="fatal" role="fatal" test="not($isSummary and ($isContinuous or $isExport or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))" diagnostics="d-IBR-140">[IBR-140-OM] - Invoice transaction type (BTOM-001) cannot be Summary invoice (00001000000000000000) if Invoice transaction type (BTOM-001) is Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000), Import of Goods (00000000000010000000).</assert>
            
            <assert id="IBR-141-OM" flag="fatal" role="fatal" test="not($isContinuous and ($isSummary or $isDeemed or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))" diagnostics="d-IBR-141">[IBR-141-OM] - Invoice transaction type (BTOM-001) cannot be Continuous supply (00000100000000000000) if Invoice transaction type (BTOM-001) is Summary invoice (00001000000000000000) or Deemed Supply Invoice (00000001000000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000).</assert>
            
            <assert id="IBR-142-OM" flag="fatal" role="fatal" test="not($isExport and ($isSelfBilled or $isSummary or $isDeemed or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isImportGoods))" diagnostics="d-IBR-142">[IBR-142-OM] - Invoice transaction type (BTOM-001) cannot be Export Invoice (00000010000000000000) if Invoice transaction type (BTOM-001) is Self-billed Invoice/credit note (00100000000000000000) or Summary invoice (00001000000000000000) or Deemed Supply Invoice (00000001000000000000) or Import of services for RCM (00000000100000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000).</assert>
            
            <assert id="IBR-143-OM" flag="fatal" role="fatal" test="not($isDeemed and ($isContinuous or $isExport or $isProfitMargin or $isProfitMarginSelf))" diagnostics="d-IBR-143">[IBR-143-OM] - Invoice transaction type (BTOM-001) cannot be Deemed Supply Invoice (00000001000000000000) if Invoice transaction type (BTOM-001) is Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000).</assert>
            
            <assert id="IBR-144-OM" flag="fatal" role="fatal" test="not($isImportRCM and ($isExport or $isProfitMargin or $isProfitMarginSelf or $isImportGoods or $isSelfBilled))" diagnostics="d-IBR-144">[IBR-144-OM] - Invoice transaction type (BTOM-001) cannot be Import of services for RCM (00000000100000000000) if Invoice transaction type (BTOM-001) is Export Invoice (00000010000000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or Import of Goods (00000000000010000000) or Self-billed Invoice/credit note (00100000000000000000).</assert>
            
            <assert id="IBR-145-OM" flag="fatal" role="fatal" test="not($isProfitMargin and ($isSummary or $isContinuous or $isExport or $isDeemed or $isImportRCM or $isImportGoods or $isSelfBilled))" diagnostics="d-IBR-145">[IBR-145-OM] - Invoice transaction type (BTOM-001) cannot be Profit margin invoice (00000000010000000000) if Invoice transaction type (BTOM-001) is Summary invoice (00001000000000000000) or Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Deemed Supply Invoice (00000001000000000000) or Import of services for RCM (00000000100000000000) or Import of Goods (00000000000010000000) or Self-billed Invoice/credit note (00100000000000000000).</assert>
            
            <assert id="IBR-146-OM" flag="fatal" role="fatal" test="not($isProfitMarginSelf and ($isSummary or $isContinuous or $isExport or $isDeemed or $isImportRCM or $isProfitMargin or $isImportGoods or $isSelfBilled))" diagnostics="d-IBR-146">[IBR-146-OM] - Invoice transaction type (BTOM-001) cannot be Profit Margin Self-Invoice (00000000001000000000) if Invoice transaction type (BTOM-001) Summary invoice (00001000000000000000) or Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Deemed Supply Invoice (00000001000000000000) or Import of services for RCM (00000000100000000000) or Profit margin invoice (00000000010000000000), Import of Goods (00000000000010000000) or Self-billed Invoice/credit note (00100000000000000000).</assert>
            
            <assert id="IBR-147-OM" flag="fatal" role="fatal" test="not($isImportGoods and ($isSummary or $isContinuous or $isExport or $isImportRCM or $isProfitMargin or $isProfitMarginSelf or $isEcommerce or $isSelfBilled))" diagnostics="d-IBR-147">[IBR-147-OM] - Invoice transaction type (BTOM-001) cannot be Import of Goods (00000000000010000000) if Invoice transaction type (BTOM-001) is Summary invoice (00001000000000000000) or Continuous supply (00000100000000000000) or Export Invoice (00000010000000000000) or Import of services for RCM (00000000100000000000) or Profit margin invoice (00000000010000000000) or Profit Margin Self-Invoice (00000000001000000000) or E-commerce supplies (00000000000100000000) or Self-billed Invoice/credit note (00100000000000000000).</assert>
            
            <assert id="IBR-148-OM" flag="fatal" role="fatal" test="not($isEcommerce and $isProfitMarginSelf)" diagnostics="d-IBR-148">[IBR-148-OM] - Invoice transaction type (BTOM-001) cannot be E-commerce supplies (00000000000100000000) if Invoice transaction type (BTOM-001) is Profit Margin Self-Invoice (00000000001000000000).</assert>
            ...
            <assert id="IBR-150-OM" flag="fatal" role="fatal" test="not($isSpecialZone)                   or (cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode and cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode)" diagnostics="d-IBR-150">[IBR-150-OM] - If Invoice transaction type (BTOM-001) is Special Zone Supplies (00000000000001000000) , buyer country subdivision code (BTOM-026) and Seller country subdivision code (BTOM-024) MUST be provided using the codelist for Country Subdivision (CL-13-OM).</assert>
            
            <assert id="IBR-151-OM" flag="fatal" role="fatal" test="                     not($isSpecialZone and cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cbc:CountrySubentityCode!='MO')                     or                     cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName='SZLN']                     " diagnostics="d-IBR-151">[IBR-151-OM] - Seller identifier (IBT-029) is mandatory with Scheme identifier (IBT-029-1) 'SZLN' (Special Zone License Number) if Invoice transaction type (BTOM-001) is Special zone supplies (00000000000001000000) and Seller country subdivision code (BTOM-024) is not equal to 'MO'.</assert>
            ...
            <assert id="IBR-153-OM" flag="fatal" role="fatal" test="                     not($isImportGoods)                     or                     cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeName='ICID']                     " diagnostics="d-IBR-153">[IBR-153-OM] - Buyer identifier (IBT-046) is mandatory with Scheme identifier (IBT-046-1) 'ICID' (Importer Customs ID) if Invoice transaction type (BTOM-001) is Import of Goods (00000000000010000000).</assert>
            
            <assert id="IBR-155-OM" flag="fatal" role="fatal" test="not($isExport and $lines[cac:Item/cac:ClassifiedTaxCategory/cbc:TaxExemptionReasonCode ='VATZR-OM-09'])                   or $lines[cac:Item/cac:ItemSpecificationDocumentReference/cbc:ID[@schemeName='MP']]" diagnostics="d-IBR-155">[IBR-155-OM] - If invoice transaction type (BTOM-001) is export invoice (00000010000000000000) and at least one VAT exemption reason code (IBT-121) is 'Export of service' then Service Type (BTOM-034) must be provided from the code list for Type of Services (CL-12-OM).</assert>
            ...
            <assert id="IBR-171-OM" flag="fatal" role="fatal" test="not(cbc:IssueDate)                     or (cbc:IssueDate castable as xs:date                         and adjust-date-to-timezone(xs:date(cbc:IssueDate), ())                             &lt;= adjust-date-to-timezone(                                     xs:date(adjust-dateTime-to-timezone(                                             current-dateTime(),                                             xs:dayTimeDuration('PT4H'))),                                     ()))" diagnostics="d-IBR-171">[IBR-171-OM] - Invoice issue date (IBT-002) MUST NOT be a future date (evaluated in Asia/Muscat, UTC+04:00).</assert>
         ...
         <rule context="cbc:ReceivedDate | cbc:InstallmentDueDate | cbc:DueDate">
        ...
        <rule context="ubl:Invoice/cac:TaxTotal/cac:TaxSubtotal | cn:CreditNote/cac:TaxTotal/cac:TaxSubtotal">
            ...
            <assert id="ALIGNED-IBRP-S-09-OM" flag="fatal" role="fatal" test="not($isInvoiceCurrency)                         or not($vatCategory = 'S')                         or u:slack($s09Actual, $s09Expected, $baisaCap)" diagnostics="d-S-09">[ALIGNED-IBRP-S-09-OM] - The VAT category tax amount (IBT-117) in a VAT breakdown (IBG-23) where VAT category code (IBT-118) is "S" MUST equal the Σ Invoice-line VAT amounts (BTOM-016) plus charges' VAT minus allowances' VAT, grouped by the VAT category rate (IBT-119) of this breakdown.</assert>
            
            <assert id="ALIGNED-IBRP-S-09-OM-WARN" flag="warning" role="warning" test="not($isInvoiceCurrency)                         or not($vatCategory = 'S')                         or u:slack($s09Actual, $s09Expected, 0)                         or not(u:slack($s09Actual, $s09Expected, $baisaCap))" diagnostics="d-S-09-WARN">[ALIGNED-IBRP-S-09-OM-WARN] - ACCEPTED WITH WARNING: The VAT category tax amount (IBT-117) for category "S" does not exactly equal the sum of invoice-line VAT amounts (BTOM-016) plus charges' VAT minus allowances' VAT, but the difference is within the permitted Baisa tolerance (+/- 0.001 OMR x number of invoice lines). The VAT breakdown is accepted; confirm the rounding is intentional.</assert>
        ...
        <rule context="cac:ClassifiedTaxCategory | cac:TaxCategory | cac:PartyTaxScheme">
            ...
            <assert id="IBR-009-OM" flag="fatal" role="fatal" test="if (exists(ancestor::cac:TaxRepresentativeParty))                             then (cac:TaxScheme/cbc:ID = 'VAT')                         else if (local-name() = 'TaxCategory')                             then (cac:TaxScheme/cbc:ID = 'VAT')                         else if ($isBuyer)                             then (count(../cac:PartyTaxScheme) = 1 and cac:TaxScheme/cbc:ID = 'VAT')                         else if ($isSupplier)                             then (                                 (cac:TaxScheme/cbc:ID = 'VAT' or ../cac:PartyTaxScheme/cac:TaxScheme/cbc:ID = 'VAT') and                                 (if (cac:TaxScheme/cbc:ID = 'VAT') then count(../cac:PartyTaxScheme[cac:TaxScheme/cbc:ID = 'VAT']) = 1 else true())                             )                         else (cac:TaxScheme/cbc:ID = 'VAT')" diagnostics="d-IBR-009">[IBR-009-OM] - Tax scheme code, must be provided in IBT-031-1 or BTOM-006-1 or IBT-048-1 or IBT-063-1 or IBT-095-1 or IBT-102-1 or IBT-118-1 or IBT-167 and shall be 'VAT'.</assert>
        </rule>
        ...
        <rule context="cac:InvoiceLine | cac:CreditNoteLine">
            ...
            <let name="ibr168Actual" value="if ($lineTaxAmount castable as xs:decimal)                         then round(xs:decimal($lineTaxAmount) * 1000) div 1000                         else ()"/>
            <let name="ibr168Expected" value="if (cbc:LineExtensionAmount castable as xs:decimal and $vatRate castable as xs:decimal)                         then round((xs:decimal(cbc:LineExtensionAmount) * xs:decimal($vatRate) div 100) * 1000) div 1000                         else ()"/>
           ...
           <let name="baseQuantity" value="                 if (cac:Price/cbc:BaseQuantity/normalize-space(.) castable as xs:decimal                      and xs:decimal(normalize-space(cac:Price/cbc:BaseQuantity)) != 0)                  then xs:decimal(normalize-space(cac:Price/cbc:BaseQuantity))                  else 1"/>
            ...
            <assert id="IBR-084-OM" flag="fatal" role="fatal" test="not($isImportGoods)                   or cac:Item/cac:OriginCountry/cbc:IdentificationCode" diagnostics="d-IBR-084">[IBR-084-OM] - If invoice transaction type (BTOM-001) is 'Import of Goods' (00000000000010000000) then Item country of origin (IBT-159) is mandatory.</assert>
            
            
            <assert id="IBR-086-OM" flag="fatal" role="fatal" test="not($isProfitMarginSelf)                   or $vatCategory='O'" diagnostics="d-IBR-086">[IBR-086-OM] - If Invoice transaction type (BTOM-001) is 'Profit Margin Self-Invoice ' (00000000001000000000), Invoiced item VAT category code (IBT-151) MUST be 'O' (Not subject to VAT).</assert>
            
            <assert id="IBR-091-OM" flag="fatal" role="fatal" test="not($isProfitMargin)                   or (every $code in cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode[@listID='HS']                       satisfies not(                            starts-with(normalize-space($code),'7101')                         or starts-with(normalize-space($code),'7102')                         or starts-with(normalize-space($code),'7103')                         or starts-with(normalize-space($code),'7104')                         or starts-with(normalize-space($code),'01')                         or starts-with(normalize-space($code),'06')))" diagnostics="d-IBR-091">[IBR-091-OM] - When Invoice transaction type (BTOM-001) is Profit margin invoice (00000000010000000000), Item classification identifier (IBT-158, listID='HS') MUST NOT start with '7101', '7102', '7103', '7104', '01' or '06'.</assert>

            
            <assert id="IBR-057-OM" flag="fatal" role="fatal" test="not($isSummary)                   or (substring(cac:InvoicePeriod/cbc:StartDate,1,7)                       = substring(cac:InvoicePeriod/cbc:EndDate,1,7))" diagnostics="d-IBR-057">[IBR-057-OM] - Invoice line period start date (IBT-134) and Invoice line period end date (IBT-135), when provided, must belong to the same calendar month where Invoice transaction type (BTOM-001) is a summary invoice (00001000000000000000).</assert>
            
            <assert id="IBR-038-OM" flag="fatal" role="fatal" test="$isSimplified or $lineTaxAmount" diagnostics="d-IBR-038">[IBR-038-OM, IBR-068-OM] - An Invoice must contain line Item VAT Amount (BTOM-016) except where invoice transaction type (BTOM-001) is a simplified tax invoice (01000000000000000000).</assert>
            ...
            <assert id="IBR-168-OM" flag="fatal" role="fatal" test="                     not($vatRate)                     or u:slack($ibr168Actual, $ibr168Expected, $baisaTolerance)                 " diagnostics="d-IBR-168">[IBR-168-OM] - The Line Item VAT amount (BTOM-016) must be Invoiced item VAT rate (IBT-152) multiplied by Invoice Line Net Amount (IBT-131).</assert>
            
            <assert id="IBR-168-OM-WARN" flag="warning" role="warning" test="                     not($vatRate)                     or u:slack($ibr168Actual, $ibr168Expected, 0)                     or not(u:slack($ibr168Actual, $ibr168Expected, $baisaTolerance))                 " diagnostics="d-IBR-168-WARN">[IBR-168-OM-WARN] - ACCEPTED WITH WARNING: The Line Item VAT amount (BTOM-016) does not exactly equal the calculation (Invoice Line Net Amount (IBT-131) x VAT rate (IBT-152) / 100), but the difference is within the permitted Baisa tolerance (+/- 0.001 OMR per line). The invoice line is accepted; confirm the rounding is intentional.</assert>
            
            <assert id="IBR-158-OM" flag="fatal" role="fatal" test="$isProfitMargin                         or ($isSimplified and not($lineTaxAmount))                         or (                             cac:ItemPriceExtension/cbc:Amount castable as xs:decimal                             and cbc:LineExtensionAmount castable as xs:decimal                             and u:slack(                                 xs:decimal(cac:ItemPriceExtension/cbc:Amount),                                 xs:decimal(cbc:LineExtensionAmount) + (if ($lineTaxAmount castable as xs:decimal) then xs:decimal($lineTaxAmount) else xs:decimal(0)),                                 $amountTolerance                             )                         )" diagnostics="d-IBR-158">[IBR-158-OM] - Total amount including VAT (BTOM-017) must be the sum of Invoice line net amount (IBT-131) and Line Item VAT amount (BTOM-016) unless if Invoice transaction type (BTOM-001) is Profit margin invoice (00000000010000000000) in which case Total amount including VAT (BTOM-017) must be the total sale value of the item.</assert>
    ...
    <pattern id="AlignedCodelists">
        ...
        <rule context="cac:AdditionalItemProperty/cbc:NameCode">
            ...
            <assert diagnostics="d-CL-11-OM" id="CL-11-OM" flag="fatal" role="fatal" test="                 (not($isProfitMargin or $isProfitMarginSelf))                 or                 (                     normalize-space(.) != ''                     and not(contains(normalize-space(.), ' '))                     and contains(                         ' VATPM-OM-01 VATPM-OM-02 VATPM-OM-03 VATPM-OM-04 VATPM-OM-05 ',                         concat(' ', normalize-space(.), ' ')                     )                 )                 ">[CL-11-OM] - If invoice transaction type (BTOM-001) is 'Profit Margin Self-Invoice' (00000000001000000000) or Profit margin invoice (00000000010000000000), Profit margin item reason code (BTOM-025) MUST be present and coded using Profit Margin Items Codelist (CL-11-OM).</assert>
        </rule>
    ...
    <diagnostics>
        ...
        <diagnostic id="d-S-09-WARN">
            ACCEPTED WITH WARNING — VAT breakdown (IBG-23) Category 'S' tolerance applied.

            Found:
            - VAT amount (IBT-117): '<value-of select="cbc:TaxAmount"/>'
            - Expected VAT amount (2-dec): '<value-of select="$s09Expected"/>'
            - Number of invoice lines: '<value-of select="$lineCount"/>'
            - Tolerance cap (0.001 × lines): '<value-of select="$baisaCap"/>'
        ...
        <diagnostic id="d-IBR-058">
            ...
            Originator ID present: '<value-of select="exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:ID)"/>'
            Value: '<value-of select="cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:ID"/>'

            Originator UUID present: '<value-of select="exists(cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:UUID)"/>'
            Value: '<value-of select="cac:AdditionalDocumentReference[cbc:DocumentTypeCode='PDR']/cbc:UUID"/>'
        </diagnostic>
       ...
       <diagnostic id="d-IBR-153">
        ...
        <diagnostic id="d-IBR-168">
            ...
            - VAT rate (IBT-152): '<value-of select="cac:Item/cac:ClassifiedTaxCategory/cbc:Percent"/>'.
            - VAT amount (BTOM-016): '<value-of select="cac:ItemPriceExtension/cac:TaxTotal/cbc:TaxAmount"/>'.
            - Expected VAT amount (3-dec): '<value-of select="$ibr168Expected"/>'.
            - Per-line tolerance (0.001): '<value-of select="$baisaTolerance"/>'.
        ...
        <diagnostic id="d-IBR-171">
            ...
            - Current date (Asia/Muscat): '<xsl:value-of select="adjust-date-to-timezone(xs:date(adjust-dateTime-to-timezone(current-dateTime(), xs:dayTimeDuration('PT4H'))),())"/>'
