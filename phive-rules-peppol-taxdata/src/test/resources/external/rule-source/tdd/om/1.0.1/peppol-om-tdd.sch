<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:pxc="urn:peppol:xslt:custom-function" queryBinding="xslt2">
  ...
  <pattern id="default">
    ...
    <rule context="/pxs:TaxData">
      ...
      <assert id="ibr-tdd-05" flag="fatal" test="matches(normalize-space(cbc:IssueTime), '^(([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]|24:00:00)(\.[0-9]+)?(Z|[+-][0-9]{2}:[0-9]{2})?$')">[ibr-tdd-05] The Tax Data Document issue time (tdt-005) MUST be a valid Issue time (as per UBL recommendations)</assert>
    ...
    <rule context="/pxs:TaxData/pxs:ReportedTransaction/pxs:ReportedDocument/cac:AccountingSupplierParty/cac:Party/cac:PartyLegalEntity">
      <assert id="ibr-tdd-61" flag="fatal" test="every $child in ('CompanyID', 'RegistrationDate', 'RegistrationExpirationDate', 'CompanyLegalFormCode', 'CompanyLegalForm', 'SoleProprietorshipIndicator', 'CompanyLiquidationStatusCode', 'CorporateStockAmount',  'FullyPaidSharesIndicator', 'RegistrationAddress', 'CorporateRegistrationScheme', 'HeadOfficeParty', 'ShareholderParty')                                                     satisfies count (*[local-name(.) = $child]) = 0">[ibr-tdd-61] Only XML elements defined in this specification are allowed to be used</assert>
        
        <assert id="ibr-tdd-62" flag="fatal" test="exists(cbc:RegistrationName)">[ibr-tdd-62] The Seller name (ibt-027) MUST be present</assert>
    </rule>
    ...
    <rule context="/pxs:TaxData/pxs:ReportedTransaction/pxs:ReportedDocument/cac:AccountingCustomerParty/cac:Party/cac:PartyLegalEntity">
      <assert id="ibr-tdd-63" flag="fatal" test="every $child in ('CompanyID', 'RegistrationDate', 'RegistrationExpirationDate', 'CompanyLegalFormCode', 'CompanyLegalForm', 'SoleProprietorshipIndicator', 'CompanyLiquidationStatusCode', 'CorporateStockAmount',  'FullyPaidSharesIndicator', 'RegistrationAddress', 'CorporateRegistrationScheme', 'HeadOfficeParty', 'ShareholderParty')                                                     satisfies count (*[local-name(.) = $child]) = 0">[ibr-tdd-63] Only XML elements defined in this specification are allowed to be used</assert>
        
        <assert id="ibr-tdd-64" flag="fatal" test="exists(cbc:RegistrationName)">[ibr-tdd-64] The Buyer name (ibt-044) MUST be present</assert>
    </rule>
