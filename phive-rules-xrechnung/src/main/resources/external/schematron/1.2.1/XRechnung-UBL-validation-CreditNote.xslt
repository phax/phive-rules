...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="" title="XRechnung 1.1 - Schematron - UBL - CreditNote">
      <xsl:comment>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>XRechnung 1.1 - Schematron - UBL - CreditNote</svrl:text>

<!--PATTERN UBL-model-->


	<!--RULE -->
<xsl:template match="//cn:CreditNote" mode="M8" priority="1006">
...
<xsl:choose>
      ...
      <xsl:otherwise>
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="cbc:CreditNoteTypeCode = ('326', '380', '384', '389', '381')" />
      <xsl:otherwise>
        <svrl:failed-assert test="cbc:CreditNoteTypeCode = ('326', '380', '384', '389', '381')">
          <xsl:attribute name="id">BR-DE-17</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>[BR-DE-17] Mit dem Element "Invoice type code" (BT-3) sollen ausschließlich folgende Codes aus der Codeliste UNTDID 1001 übermittelt werden: 326 (Partial invoice), 380 (Commercial invoice), 384 (Corrected invoice), 389 (Self-billed invoice) und 381 (Credit note).</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cn:CreditNote/cac:AccountingSupplierParty" mode="M8" priority="1005">
    <svrl:fired-rule context="//cn:CreditNote/cac:AccountingSupplierParty" />
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cn:CreditNote/cac:AccountingSupplierParty/cac:Party/cac:PostalAddress" mode="M8" priority="1004">
    <svrl:fired-rule context="//cn:CreditNote/cac:AccountingSupplierParty/cac:Party/cac:PostalAddress" />
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cn:CreditNote/cac:AccountingSupplierParty/cac:Party/cac:Contact" mode="M8" priority="1003">
    <svrl:fired-rule context="//cn:CreditNote/cac:AccountingSupplierParty/cac:Party/cac:Contact" />
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cn:CreditNote/cac:AccountingCustomerParty/cac:Party/cac:PostalAddress" mode="M8" priority="1002">
    <svrl:fired-rule context="//cn:CreditNote/cac:AccountingCustomerParty/cac:Party/cac:PostalAddress" />
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cn:CreditNote/cac:Delivery/cac:DeliveryLocation/cac:Address" mode="M8" priority="1001">
    <svrl:fired-rule context="//cn:CreditNote/cac:Delivery/cac:DeliveryLocation/cac:Address" />
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cn:CreditNote/cac:TaxTotal/cac:TaxSubtotal" mode="M8" priority="1000">
    <svrl:fired-rule context="//cn:CreditNote/cac:TaxTotal/cac:TaxSubtotal" />
