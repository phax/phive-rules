...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="ubl:Invoice/cac:InvoiceLine/cac:DocumentReference/cbc:ID | cn:CreditNote/cac:CreditNoteLine/cac:DocumentReference/cbc:ID" mode="M22" priority="1000">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(custom:isSpecialContract(/ubl:Invoice|/cn:CreditNote)) or (abs(number(../cac:TaxTotal/cbc:TaxAmount[@currencyID = $invcurrency]) - $sumvat) &lt;= 0.00)">
