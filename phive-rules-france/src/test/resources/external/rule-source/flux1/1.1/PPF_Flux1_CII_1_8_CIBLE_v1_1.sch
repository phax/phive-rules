...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2" schemaVersion="ISO19757-3">
  <title>PPF — Flux 1 CII e-invoicing &amp; e-reporting — Profil de Base (Trajectoire DÉMARRAGE) - Extension FULL/CIBLE</title>
  ...
  <pattern id="F1-START-VENDEUR-PAYS-G2.01">
    <rule context="/rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeAgreement/ram:SellerTradeParty/ram:PostalTradeAddress/ram:CountryID">
      <assert test="matches(., '^[A-Z]{2}$')" flag="fatal">[G2.01] Le code pays du vendeur (<value-of select="."/>) doit comporter 2 lettres majuscules ISO 3166-1 alpha-2 ('EL' requis pour la Grèce). | Source : Annexe 7 v1.8 G2.01</assert>
    </rule>
  </pattern>
  ...
  <pattern id="F1-START-TOTAL-COHERENCE-G1.53">
    <rule context="/rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement">
      ...
      <let name="deviseFacture" value="ram:InvoiceCurrencyCode"/>
      <let name="totalTVA" value="ram:SpecifiedTradeSettlementHeaderMonetarySummation/ram:TaxTotalAmount[@currencyID = $deviseFacture]"/>
      <let name="sommeTVA" value="ram:ApplicableTradeTax/ram:CalculatedAmount"/>
      <let name="sommeBasesHTDec" value="for $x in $sommeBasesHT return xs:decimal($x)"/>
      <let name="sommeTVADec" value="for $x in $sommeTVA return xs:decimal($x)"/>
      <assert test="not($totalHT) or not($sommeBasesHT) or (abs(xs:decimal($totalHT) - sum($sommeBasesHTDec)) &lt;= 0.01)" flag="fatal">[G1.53] Écart sur le Total Hors Taxe global (<value-of select="$totalHT"/>) vis-à-vis de la somme des bases ventilées (<value-of select="sum($sommeBasesHTDec)"/>). Tolérance de 0,01 EUR. | Source : Annexe 7 v1.8 G1.53</assert>
      <assert test="not($totalTVA) or not($sommeTVA) or (abs(xs:decimal($totalTVA) - sum($sommeTVADec)) &lt;= 0.01)" flag="fatal">[G1.53] Écart sur le Montant Total de TVA global (<value-of select="$totalTVA"/>) vis-à-vis de la somme des lignes de TVA calculées (<value-of select="sum($sommeTVADec)"/>). Tolérance de 0,01 EUR. | Source : Annexe 7 v1.8 G1.53</assert>
    </rule>
  </pattern>
  ...
  <pattern id="F1-COMPLETE-CII-G1.10-G1.12-DEVISES">
    <rule context="/rsm:CrossIndustryInvoice">
      ...
      <let name="deviseBT5" value="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:InvoiceCurrencyCode"/>
      <let name="occurrencesBT110" value="rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:SpecifiedTradeSettlementHeaderMonetarySummation/ram:TaxTotalAmount"/>
      <assert test="($deviseBT5 = 'EUR' and count($occurrencesBT110) = 1 and $occurrencesBT110[@currencyID = 'EUR']) or ($deviseBT5 != 'EUR' and count($occurrencesBT110) = 2 and $occurrencesBT110[@currencyID = $deviseBT5] and $occurrencesBT110[@currencyID = 'EUR'])" flag="fatal">[G1.12] Si BT-5 est different de EUR, le document doit comporter exactement 2 occurrences du montant total de TVA (BT-110/BT-111) : une en devise BT-5 et une en EUR. Si BT-5 est EUR, une seule occurrence en EUR est attendue. | Source : Annexe 7 v1.8 G1.12</assert>
    </rule>
  </pattern>
  ...
  <pattern id="F1-FULL-CII-G6.09-LIGNES-OBLIGATOIRES">
    <rule context="/rsm:CrossIndustryInvoice">
      <assert test="rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem" flag="fatal">[G6.09] BG-25 Ligne de facture obligatoire en profil FULL/CIBLE. Extension du profil BASE. | Source : Annexe 1 Flux 1 / Annexe 7 G6.09</assert>
    </rule>
  </pattern>

  <pattern id="F1-FULL-CII-G6.09-CONTENU-LIGNE">
    <rule context="/rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem">
      <assert test="ram:SpecifiedLineTradeDelivery/ram:BilledQuantity" flag="fatal">[G6.09] BT-129 Quantite facturee obligatoire en profil FULL/CIBLE. | Source : Annexe 1 Flux 1 / Annexe 7 G6.09</assert>
      <assert test="ram:SpecifiedLineTradeDelivery/ram:BilledQuantity/@unitCode" flag="fatal">[G6.09] BT-130 Unite de quantite obligatoire en profil FULL/CIBLE. | Source : Annexe 1 Flux 1 / Annexe 7 G6.09</assert>
      <assert test="ram:SpecifiedLineTradeAgreement" flag="fatal">[G6.09] BG-29 Detail du prix obligatoire en profil FULL/CIBLE. | Source : Annexe 1 Flux 1 / Annexe 7 G6.09</assert>
      <assert test="ram:SpecifiedLineTradeAgreement/ram:NetPriceProductTradePrice/ram:ChargeAmount" flag="fatal">[G6.09] BT-146 Prix net unitaire obligatoire en profil FULL/CIBLE. | Source : Annexe 1 Flux 1 / Annexe 7 G6.09</assert>
      <assert test="ram:SpecifiedLineTradeAgreement/ram:GrossPriceProductTradePrice/ram:ChargeAmount" flag="fatal">[G6.09] BT-148 Prix brut unitaire obligatoire en profil FULL/CIBLE. | Source : Annexe 1 Flux 1 / Annexe 7 G6.09</assert>
      <assert test="ram:SpecifiedTradeProduct" flag="fatal">[G6.09] BG-31 Article obligatoire en profil FULL/CIBLE. | Source : Annexe 1 Flux 1 / Annexe 7 G6.09</assert>
      <assert test="ram:SpecifiedTradeProduct/ram:Name" flag="fatal">[G6.09] BT-153 Nom de l'article obligatoire en profil FULL/CIBLE. | Source : Annexe 1 Flux 1 / Annexe 7 G6.09</assert>
    </rule>
  </pattern>

  <pattern id="F1-CIBLE-CII-G1.31-REFERENCES-ANTERIEURES">
    <rule context="/rsm:CrossIndustryInvoice">
      <assert test="not(rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:InvoiceReferencedDocument/ram:IssuerAssignedID and rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem/ram:SpecifiedLineTradeSettlement/ram:InvoiceReferencedDocument/ram:IssuerAssignedID)" flag="fatal">[G1.31] Les references a factures anterieures ne doivent pas etre renseignees simultanement en entete et en ligne en profil FULL/CIBLE. | Source : Annexe 7 v1.8 G1.31</assert>
      <assert test="not(rsm:ExchangedDocument/ram:TypeCode = ('261','381','384','396','471','472','473','502','503')) or (rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:InvoiceReferencedDocument/ram:IssuerAssignedID and rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:InvoiceReferencedDocument/ram:FormattedIssueDateTime/qdt:DateTimeString) or (every $l in rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem satisfies ($l/ram:SpecifiedLineTradeSettlement/ram:InvoiceReferencedDocument/ram:IssuerAssignedID and $l/ram:SpecifiedLineTradeSettlement/ram:InvoiceReferencedDocument/ram:FormattedIssueDateTime/qdt:DateTimeString))" flag="fatal">[G1.31] Pour un avoir ou une facture rectificative en FULL/CIBLE, la reference anterieure doit porter le numero et la date, soit en entete soit sur toutes les lignes. | Source : Annexe 7 v1.8 G1.31</assert>
      <assert test="not(rsm:ExchangedDocument/ram:TypeCode = ('384','471','472','473')) or count(rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeSettlement/ram:InvoiceReferencedDocument/ram:IssuerAssignedID) &lt;= 1" flag="fatal">[G1.31] Une facture rectificative doit pointer au plus une facture anterieure en entete. | Source : Annexe 7 v1.8 G1.31</assert>
    </rule>
  </pattern>

  <pattern id="F1-CIBLE-CII-BT26-DATE-FACTURE-ANTERIEURE">
    <rule context="/rsm:CrossIndustryInvoice//ram:InvoiceReferencedDocument/ram:FormattedIssueDateTime/qdt:DateTimeString">
      <assert test="matches(., '^\d{8}$')" flag="fatal">[G1.09] BT-26 Date de facture anterieure au format CII AAAAMMJJ. | Source : Annexe 1 Flux 1 / Annexe 7 G1.09</assert>
      <assert test="not(matches(., '^\d{8}$')) or (number(substring(., 1, 4)) &gt;= 2000 and number(substring(., 1, 4)) &lt;= 2099)" flag="fatal">[G1.36] BT-26 L'annee de la facture anterieure doit etre comprise entre 2000 et 2099. | Source : Annexe 7 v1.8 G1.36</assert>
    </rule>
  </pattern>

  <pattern id="F1-CIBLE-CII-G6.16-ADRESSE-LIVRAISON">
    <rule context="/rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:ApplicableHeaderTradeDelivery/ram:ShipToTradeParty/ram:PostalTradeAddress | /rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem/ram:SpecifiedLineTradeDelivery/ram:ShipToTradeParty/ram:PostalTradeAddress">
      <assert test="ram:LineOne" flag="fatal">[G6.16] BT-75 Ligne d'adresse de livraison obligatoire lorsque l'adresse de livraison est renseignee. | Source : Annexe 1 Flux 1 / Annexe 7 G6.16</assert>
      <assert test="ram:CityName" flag="fatal">[G6.16] BT-77 Ville de livraison obligatoire lorsque l'adresse de livraison est renseignee. | Source : Annexe 1 Flux 1 / Annexe 7 G6.16</assert>
      <assert test="ram:PostcodeCode" flag="fatal">[G6.16] BT-78 Code postal de livraison obligatoire lorsque l'adresse de livraison est renseignee. | Source : Annexe 1 Flux 1 / Annexe 7 G6.16</assert>
      <assert test="ram:CountryID" flag="fatal">[G6.16] BT-80 Code pays de livraison obligatoire lorsque l'adresse de livraison est renseignee. | Source : Annexe 1 Flux 1 / Annexe 7 G6.16</assert>
      <assert test="not(ram:CountryID) or (matches(ram:CountryID, '^[A-Z]{2}$') and not(ram:CountryID = 'EL'))" flag="fatal">[G2.01] Le code pays de livraison doit etre au format ISO 3166-1 alpha-2, avec GR pour la Grece. | Source : Annexe 7 v1.8 G2.01</assert>
    </rule>
  </pattern>

  <pattern id="F1-CIBLE-CII-DATES-LIGNE">
    <rule context="/rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem/ram:SpecifiedLineTradeDelivery/ram:ActualDeliverySupplyChainEvent/ram:OccurrenceDateTime/udt:DateTimeString | /rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem/ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod/ram:StartDateTime/udt:DateTimeString | /rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem/ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod/ram:EndDateTime/udt:DateTimeString">
      <assert test="matches(., '^\d{8}$')" flag="fatal">[G1.09] Les dates de ligne FULL/CIBLE doivent respecter le format CII AAAAMMJJ. | Source : Annexe 7 v1.8 G1.09</assert>
      <assert test="not(matches(., '^\d{8}$')) or (number(substring(., 1, 4)) &gt;= 2000 and number(substring(., 1, 4)) &lt;= 2099)" flag="fatal">[G1.36] L'annee des dates de ligne FULL/CIBLE doit etre comprise entre 2000 et 2099. | Source : Annexe 7 v1.8 G1.36</assert>
    </rule>
  </pattern>

  <pattern id="F1-CIBLE-CII-G6.25-PERIODES-LIGNE">
    <rule context="/rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem/ram:SpecifiedLineTradeSettlement/ram:BillingSpecifiedPeriod[ram:StartDateTime/udt:DateTimeString and ram:EndDateTime/udt:DateTimeString]">
      <assert test="ram:EndDateTime/udt:DateTimeString &gt; ram:StartDateTime/udt:DateTimeString" flag="fatal">[G6.25] En ligne, la date de fin de periode ne peut pas etre anterieure ou egale a la date de debut. | Source : Annexe 7 v1.8 G6.25</assert>
    </rule>
  </pattern>

<pattern id="F1-FULL-CII-ALLOWANCE-CHARGE-DOCUMENT">
  <rule context="/rsm:CrossIndustryInvoice/
                 rsm:SupplyChainTradeTransaction/
                 ram:ApplicableHeaderTradeSettlement/
                 ram:SpecifiedTradeAllowanceCharge">

    <assert test="ram:ActualAmount" flag="fatal">
    ...
    <assert test="ram:CategoryTradeTax/ram:CategoryCode" flag="fatal">
      [G6.09] Code de catégorie TVA obligatoire sur remise/charge document.
      | Source : Annexe 1 Flux 1
    </assert>

    <assert
      test="ram:ChargeIndicator/udt:Indicator = ('true', '1')
            or normalize-space(ram:CategoryTradeTax/ram:RateApplicablePercent) != ''"
      flag="fatal">
      [G6.10] Le taux de TVA est obligatoire pour une remise au niveau du document.
      | Source : Annexe 7 v1.8 G6.10
    </assert>

    <assert test="ram:CategoryTradeTax/ram:TypeCode = 'VAT'" flag="fatal">
      [S1.17] Le qualifiant TVA des remises/charges document doit être VAT.
      | Source : Annexe 7 v1.8 S1.17
    </assert>

    <assert
      test="not(ram:CategoryTradeTax/ram:CategoryCode)
            or ram:CategoryTradeTax/ram:CategoryCode =
               ('S','E','AE','K','G','O','Z')"
      flag="fatal">
    ...
    <assert
      test="not(ram:CategoryTradeTax/ram:RateApplicablePercent)
            or number(ram:CategoryTradeTax/ram:RateApplicablePercent) =
  ...
  <pattern id="F1-FULL-CII-ALLOWANCE-CHARGE-LIGNE">
    <rule context="/rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem/ram:SpecifiedLineTradeSettlement/ram:SpecifiedTradeAllowanceCharge">
      <assert test="ram:ActualAmount" flag="fatal">[G6.09] Montant de remise/charge de ligne obligatoire lorsque BG-27/BG-28 est renseigne. | Source : Annexe 1 Flux 1</assert>
    </rule>
  </pattern>

  <pattern id="F1-FULL-CII-G1.16-PRIX-LIGNE">
    <rule context="/rsm:CrossIndustryInvoice/rsm:SupplyChainTradeTransaction/ram:IncludedSupplyChainTradeLineItem/ram:SpecifiedLineTradeAgreement//ram:*[local-name() = ('ChargeAmount','ActualAmount','BasisAmount')]">
      <assert test="matches(string(.), '^\d+(\.\d{1,6})?$')" flag="fatal">[G1.16] Le prix doit comporter au maximum 6 decimales, sans signe negatif. | Source : Annexe 7 v1.8 G1.16</assert>
      <assert test="string-length(translate(string(.), '.', '')) &lt;= 19" flag="fatal">[G1.16] Le prix ne peut pas depasser 19 chiffres hors separateur. | Source : Annexe 7 v1.8 G1.16</assert>
    </rule>
  </pattern>
