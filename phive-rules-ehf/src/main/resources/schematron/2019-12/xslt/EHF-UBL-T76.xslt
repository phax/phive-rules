...
<!--PATTERN -->
<xsl:variable name="clUNCL1225" select="tokenize('27 29 30', '\s')" />
...
<xsl:template match="/ubl:OrderResponse/cbc:OrderResponseCode" mode="M4" priority="1070">
    <svrl:fired-rule context="/ubl:OrderResponse/cbc:OrderResponseCode" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="(some $code in $clUNCL1225 satisfies $code = normalize-space(text()))" />
      <xsl:otherwise>
        <svrl:failed-assert test="(some $code in $clUNCL1225 satisfies $code = normalize-space(text()))">
          <xsl:attribute name="id">EHF-T76-B00801</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>Value MUST be part of code list 'Response Code (Order Response)'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="cac:Party">
          <xsl:attribute name="id">EHF-T76-B02901</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
