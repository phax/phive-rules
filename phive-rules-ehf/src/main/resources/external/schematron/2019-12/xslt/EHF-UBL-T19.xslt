...
<!--PATTERN -->
<xsl:variable name="clActioncodedocumentlevel" select="tokenize('Add Replace Update Delete', '\s')" />
  <xsl:variable name="cllabels" select="tokenize('BAL GEA BBH-AC BBH-RE NEO EBO ESU EEL EEA FSC GOT LBA MEC MSC PEF RAC RCL SML-A SML-B TCL BCI FFF FTO FBL FAC MBL MHL RSB RCS SAN TCL UCL Difi1s Difi2s Difi3s Difi4s Difi5s Difi6s Difi7s Difi8s ABA DOM DBL EOP KBL SAO Difi1o BBS-Y BBS-R BBL Difi1a Difi999', '\s')" />
...
<xsl:template match="/ubl:Catalogue/cbc:ActionCode" mode="M4" priority="1203">
    <svrl:fired-rule context="/ubl:Catalogue/cbc:ActionCode" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="(some $code in $clActioncodedocumentlevel satisfies $code = normalize-space(text()))" />
      <xsl:otherwise>
        <svrl:failed-assert test="(some $code in $clActioncodedocumentlevel satisfies $code = normalize-space(text()))">
          <xsl:attribute name="id">EHF-T19-B00601</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>Value MUST be part of code list 'Action code (Document level)'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="@listID">
          <xsl:attribute name="id">EHF-T19-B00602</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(some $code in $clGS17009 satisfies $code = normalize-space(text()))">
          ...
          <svrl:text>Value MUST be part of code list 'Package Level'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="@listID">
          <xsl:attribute name="id">EHF-T19-B13402</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
...
<xsl:template match="/ubl:Catalogue/cac:CatalogueLine/cac:Item/cac:Certificate/cbc:CertificateTypeCode" mode="M4" priority="1013">
    <svrl:fired-rule context="/ubl:Catalogue/cac:CatalogueLine/cac:Item/cac:Certificate/cbc:CertificateTypeCode" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="(some $code in $cllabels satisfies $code = normalize-space(text()))" />
      <xsl:otherwise>
        <svrl:failed-assert test="(some $code in $cllabels satisfies $code = normalize-space(text()))">
          <xsl:attribute name="id">EHF-T19-B26401</xsl:attribute>
          <xsl:attribute name="flag">warning</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>Value MUST be part of code list 'Environmental and Social Labels'.</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:apply-templates mode="M4" select="*" />
