...
<!--PATTERN UBL-T14-->
	...
	<!--RULE -->
<xsl:template match="/ubl:CreditNote/cac:AllowanceCharge" mode="M7" priority="1007">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="((/ubl:CreditNote/cac:TaxTotal/*/*/*/cbc:ID = 'VAT') and (cac:TaxCategory/cbc:ID)) or not(/ubl:CreditNote/cac:TaxTotal)">
	...
	<!--RULE -->
<xsl:template match="/ubl:CreditNote" mode="M7" priority="1006">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cac:CreditNoteLine" mode="M7" priority="1005">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not(cac:Price/cbc:PriceAmount) or (cac:Price/cbc:PriceAmount) >= 0">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cac:LegalMonetaryTotal" mode="M7" priority="1003">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(xs:decimal(cbc:LineExtensionAmount)) = (round(sum((//cac:CreditNoteLine/xs:decimal(cbc:LineExtensionAmount))) * 10 * 10) div 100)">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID = 'VAT']" mode="M7" priority="1002">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cac:TaxTotal[cac:TaxSubtotal/cac:TaxCategory/cbc:ID = 'AE']" mode="M7" priority="1001">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="//cac:CommodityClassification/cbc:ItemClassificationCode" mode="M7" priority="1000">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="(@listID)">
          ...
          <svrl:text>[BII2-T14-R033]-A list identifier for a credit note line item commodity classification MUST be provided if credit note line item commodity classification are used to classify a credit note line item (e.g. CPV or UNSPSC)</svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:apply-templates mode="M7" select="*|comment()|processing-instruction()" />
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cbc:*/@currencyID" mode="M8" priority="1005">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cbc:*/@mimeCode" mode="M8" priority="1001">
