...
<!--SCHEMA SETUP-->
<xsl:template match="/">
    <svrl:schematron-output schemaVersion="iso" title="BIIRULES  T110 bound to UBL">
      <xsl:comment>
...
<!--SCHEMATRON PATTERNS-->
<svrl:text>BIIRULES  T110 bound to UBL</svrl:text>

<!--PATTERN business_rules-->


	<!--RULE -->
<xsl:template match="ubl:OrderResponse" mode="M5" priority="1003">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cac:BuyerCustomerParty" mode="M5" priority="1002">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cac:OrderLine" mode="M5" priority="1000">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="cac:LineItem/cbc:ID and count(//cac:LineItem[normalize-space(cbc:ID) = $lineId]) = 1">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="some $code in $mime satisfies $code = normalize-space(@mimeCode)">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cbc:ItemClassificationCode[@listID]" mode="M6" priority="1001">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="( ( not(contains(normalize-space(@listID),' ')) and contains( ' CV GN HS CPV UNSPSC eCLASS ',concat(' ',normalize-space(@listID),' ') ) ) )">
	...
	<!--RULE -->
<xsl:template match="cac:ClassifiedTaxCategory/cbc:ID" mode="M6" priority="1000">
